 ...
 ******************************************************/
public class DataChainBcastReceiver extends Receiver {

    @SuppressWarnings("unused")
    private static final Logger LOG = Logger.getLogger(DataChainBcastReceiver.class);
     ...
     * @throws Exception
     */
    public DataChainBcastReceiver(int selfID, ServerConn conn, EventQueue queue, DataMap map, Workers w,
	    byte commandType) throws Exception {
	...
	if (this.getCommandType() == Constant.CHAIN_BCAST_DECODE) {
	...
	if (sourceID != nextID) {
	    nextConn = Connection.create(nextHost, nextPort, true);
	    if (nextConn == null) {
		opArray.release();
		throw new IOException("Cannot create the next connection.");
	    }
	}
	...
	if (headArray != null) {
	    try {
		IOUtil.receiveBytes(in, headArray.get(), headArray.start(), headArray.size());
		// Forward op bytes and head bytes
		if (out != null) {
		    out.write(getCommandType());
		    IOUtil.sendBytes(out, opArray.get(), opArray.start(), opArray.size());
		    IOUtil.sendBytes(out, headArray.get(), headArray.start(), headArray.size());
		}
	    } catch (Exception e) {
		headArray.release();
		if (nextConn != null) {
	...
	if (bodyArray != null) {
	    try {
		receiveBytes(in, out, bodyArray.get(), bodyArray.start(), bodyArray.size());
	    } catch (Exception e) {
		headArray.release();
		bodyArray.release();
		if (nextConn != null) {
     ...
     * @throws IOException
     */
    private void receiveBytes(InputStream in, OutputStream out, byte[] bytes, int start, int size) throws IOException {

	if (out != null) {
	    while (size > Constant.PIPELINE_SIZE) {
		int len = in.read(bytes, start, Constant.PIPELINE_SIZE);
		if (len > 0) {
