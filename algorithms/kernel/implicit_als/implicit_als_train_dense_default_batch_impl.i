...
static inline void getSizes( size_t  nRows,
                             ...
                             size_t* pBlocks,
                             size_t* pBlockSize,
                             size_t* pLastBlockSize )
{
    static size_t nThreads            = threader_get_threads_number();
    static const size_t maxBlockSize  = 100000;
    size_t minBlockSize = nCols;
    size_t defBlocks    = (nThreads > 50)?nThreads:nThreads*2;
    size_t allSize      = nRows*nCols;
    size_t blockSize    = allSize / defBlocks;

    if(blockSize > maxBlockSize)blockSize = maxBlockSize;
    if(blockSize < minBlockSize)blockSize = minBlockSize;

    *pBlockSize = blockSize/nCols;
    *pBlocks = nRows/(*pBlockSize);
    *pLastBlockSize = nRows - (*pBlocks)*(*pBlockSize);
...
{
    SafeStatus safeStat;
    size_t nBlocks, BlockSize, LastBlockSize;

    getSizes( nRows, nCols, &nBlocks, &BlockSize, &LastBlockSize );

    daal::threader_for(nBlocks, nBlocks, [ & ]( size_t i )
    {
        size_t CurBlockSize = ( i < nBlocks-1 )? BlockSize : LastBlockSize;

        for( size_t j = 0; j < CurBlockSize; j++ )
        {

            algorithmFPType *lhs_local = lhs.local();
            algorithmFPType *rhs = rowFactors + (i*BlockSize+j) * nFactors;
            ...
            formSystem( (i*BlockSize+j), nCols, data, colIndices, rowOffsets, nFactors, colFactors, alpha, lhs_local, rhs, lambda);
