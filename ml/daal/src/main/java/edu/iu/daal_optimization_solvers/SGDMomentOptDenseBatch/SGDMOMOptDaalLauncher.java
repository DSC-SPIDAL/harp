...
public class SGDMOMOptDaalLauncher extends Configured
  implements Tool {
  ...
  public static void main(String[] argv)
    ...
    int res =
      ToolRunner.run(new Configuration(),
        new SGDMOMOptDaalLauncher(), argv);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 11) {
      System.err
        .println("Usage: edu.iu.daal_svd.SGDMOMOptDaalLauncher "
          + "<file dim > "
	  + "<nfeature>"
	  + "<accuracyThreshold>"
	  + "<nIterations>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runSGDMOMOpt(nFileDim, nFeatures, accuracyThreshold, nIterations, batchSize, learningRate, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total SGDMOMOpt Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runSGDMOMOpt(
    ...
    Job SGDMOMOptJob =
      configureSGDMOMOptJob(nFileDim, nFeatures, accuracyThreshold, nIterations, batchSize, learningRate,
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      SGDMOMOptJob.waitForCompletion(true);
    System.out
      .println("end job "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("SGDMOMOpt Job fails.");
    }
  }

  private Job configureSGDMOMOptJob(
    ...
    Job job =
      Job
        .getInstance(configuration, "SGDMOMOpt_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(SGDMOMOptDaalLauncher.class);
    job
      .setMapperClass(SGDMOMOptDaalCollectiveMapper.class);
