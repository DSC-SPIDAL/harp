...
/*******************************************************
 * The actual sender for broadcasting the data using MST method We don't allow
 * the worker broadcasts to itself.
 ******************************************************/
public class DataMSTBcastSender extends DataSender {

    private static final Logger LOG = Logger.getLogger(DataMSTBcastSender.class);

    public DataMSTBcastSender(Data data, Workers workers, byte command) {
	super(data, getDestID(workers.getSelfID(), workers.getMinID(), workers.getMiddleID(), workers.getMaxID()),
		workers, command);
    }
     ...
     */
    private static int getDestID(int selfID, int left, int middle, int right) {
     ...
     * @param opArray
     *            the ByteArray storing the size of the head array
     * @param data
     *            the Data to be broadcast
     * @throws IOException
     */
    protected void sendDataBytes(Connection conn, final ByteArray opArray, final Data data) throws IOException {
	...
	Serializer serializer = new Serializer(new ByteArray(opArray.get(), 4, 8));
	try {
	...
	while (left < right) {
	    ...
	    // + destLeft + " " + destRight);
	    serializer = new Serializer(new ByteArray(opArray.get(), 4, 8));
	    try {
		serializer.writeInt(destLeft);
		serializer.writeInt(destRight);
	    } catch (IOException e) {
		LOG.error("Fail to serialize to op array.", e);
		continue;
	    }
	    // Send data to destination
	    WorkerInfo destWorker = getWorkers().getWorkerInfo(destID);
	    if (destWorker != null) {
		Connection destConn = Connection.create(destWorker.getNode(), destWorker.getPort(), true);
		if (destConn != null) {
		    try {
			super.sendDataBytes(destConn, opArray, data);
			destConn.release();
		    } catch (IOException e) {
			LOG.error("Fail to send data bytes.", e);
			destConn.free();
