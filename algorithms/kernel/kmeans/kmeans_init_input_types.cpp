...
services::Status checkLocalData(const DataCollection* pInput, const Parameter* par, const char* dataName,
    const NumericTable* pData, bool bParallelPlus)
{
    ...
    DAAL_CHECK_EX(pInput->size() == (bParallelPlus ? localDataSize : localDataSize - 1),
        ErrorIncorrectDataCollectionSize, ArgumentName, dataName);
