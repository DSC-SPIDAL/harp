 ...
 * \file updater_HistMakerCompactHalfTrick.cc
...
DMLC_REGISTRY_FILE_TAG(updater_pmathalftrick);

template<typename TStats>
class HistMakerCompactHalfTrick: public BaseMaker {
 public:
...
template<typename TStats>
class CQHistMakerCompactHalfTrick: public HistMakerCompactHalfTrick<TStats> {
 public:
  CQHistMakerCompactHalfTrick()  = default;

 protected:
  struct HistEntry {
    typename HistMakerCompactHalfTrick<TStats>::HistUnit hist;
    unsigned istart;
...
// global proposal
template<typename TStats>
class GlobalProposalHistMakerCompactHalfTrick: public CQHistMakerCompactHalfTrick<TStats> {

 public:
  GlobalProposalHistMakerCompactHalfTrick(){
 ...
 protected:
  void ResetPosAndPropose(const std::vector<GradientPair> &gpair,
      ...
      if(!this->isInitializedHistIndex){
        ...
        CQHistMakerCompactHalfTrick<TStats>::ResetPosAndPropose(gpair, p_fmat, fset, tree);
        LOG(CONSOLE) << "ResetPosAndPropose call in globalproposal";

        //this->wspace_.Init(this->param_, 1, 1);
        //CQHistMakerCompactHalfTrick<TStats>::InitHistIndex(p_fmat, fset, tree);
...
XGBOOST_REGISTER_TREE_UPDATER(HistMakerCompactHalfTrick, "grow_pmathalftrick")
.describe("Tree constructor that uses approximate global of histogram construction.")
.set_body([]() {
    return new GlobalProposalHistMakerCompactHalfTrick<GradStats>();
  });
}  // namespace tree
}  // namespace xgboost
