...
public class IOUtil {
     ...
     * @throws IOException
     */
    public static void sendBytes(final OutputStream out, final byte[] bytes, int start, int size) throws IOException {
     ...
     */
    public static void receiveBytes(InputStream in, byte[] bytes, int start, int size) throws IOException {
	while (size > 0) {
     ...
     * @throws SocketException
     */
    public static void setServerSocketOptions(ServerSocket socket) throws SocketException {
	// int recvSize1 =
	// socket.getReceiveBufferSize();
	socket.setReuseAddress(true);
	socket.setReceiveBufferSize(Constant.BUFFER_SIZE);
     ...
     */
    public static Data waitAndGet(DataMap dataMap, String contextName, String operationName) {
	int count = 0;
	do {
	    try {
		return dataMap.waitAndGetData(contextName, operationName, Constant.DATA_MAX_WAIT_TIME);
	    } catch (InterruptedException e) {
		if (count == Constant.SMALL_RETRY_COUNT) {
		    return null;
		}
		count++;
		LOG.info("Retry. context name: " + contextName + ", operationName: " + operationName);
	    }
	} while (true);
    }
}
