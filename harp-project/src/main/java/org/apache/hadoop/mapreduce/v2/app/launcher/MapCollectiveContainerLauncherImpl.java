...
/*******************************************************
 * This class is modified from ContainerLauncherImpl. When launch event is
 * processed, record the location and generate nodes file. Max mem is also reset
 * for mapper in map-collective job.
 *
 * The following is the original document.
 *
 * This class is responsible for launching of containers.
 ******************************************************/
public class MapCollectiveContainerLauncherImpl extends ContainerLauncherImpl {

    static final Log LOG = LogFactory.getLog(MapCollectiveContainerLauncherImpl.class);

    static final String MAP_COLLECTIVE_JAVA_OPTS = "mapreduce.map.collective.java.opts";
     ...
     */
    class MapCollectiveEventProcessor extends EventProcessor {
	private ContainerLauncherEvent event;

	MapCollectiveEventProcessor(ContainerLauncherEvent event) {
	    super(event);
	...
	public void run() {
	    ...
	    if (event.getType() == ContainerLauncher.EventType.CONTAINER_REMOTE_LAUNCH
		    && event.getTaskAttemptID().getTaskId().getTaskType() == TaskType.MAP) {
    ...
    /**
     * The event is generated in TaskAttemptImpl.ContainerAssignedTransition.
     * The event is edited with the new map collective java opts.
     * 
     * @param event
     */
    private void editMapCollectiveLaunchEvent(ContainerRemoteLaunchEvent launchEvent) {
	try {
	    ...
	    Container container = launchEvent.getAllocatedContainer();
	    long conID = container.getId().getContainerId();
	    int conMem = container.getResource().getMemory();
	    int conCore = container.getResource().getVirtualCores();
	    // Get host and rack
	    // Use ContainerLauncherImpl's configuration
	    String host = container.getNodeId().getHost();
	    RackResolver.init(getConfig());
	    String nodeRackName = RackResolver.resolve(host).getNetworkLocation();
	    ...
	    // from task
	    String javaOptsStr = task.getConf().get(JobConf.MAPRED_MAP_TASK_JAVA_OPTS,
		    JobConf.DEFAULT_MAPRED_TASK_JAVA_OPTS);
	    // Based on the code in
	    // MapReduceChildJVM.getVMCommand to get
	    // java opts
	    javaOptsStr = javaOptsStr.replace("@taskid@", attemptID.toString()).trim();
	    String[] javaOptsSplit = javaOptsStr.split("[\\s]+");
	    ...
	    // java opts
	    String mcJavaOptsStr = task.getConf().get(MAP_COLLECTIVE_JAVA_OPTS, "");
	    mcJavaOptsStr = mcJavaOptsStr.replace("@taskid@", attemptID.toString()).trim();
	    // Record task locations
	    LOG.info("Container launch info: " + "task_id = " + taskID + ", " + "task_type = Map" + ", " + "host = "
		    + host + ", " + "rack = " + nodeRackName + ", " + "container ID = " + conID + ", "
		    + "container mem = " + conMem + ", " + "container core = " + conCore + ", " + "java new opts = "
		    + mcJavaOptsStr);
	    ...
	    // commands
	    String commands = launchContext.getCommands().get(0);
		...
		for (int i = 1; i < javaOpts.size(); i++) {
		    newCommands = newCommands.replace(javaOpts.get(i), "");
		}
		// Replace the first one (0) to new java
		// opt string
		newCommands = newCommands.replace(javaOpts.get(0), mcJavaOptsStr);
	    }
	    // LOG.info("New commands: " + newCommands);
	    List<String> finalNewCommands = new ArrayList<>();
	    finalNewCommands.add(newCommands);
	    launchContext.setCommands(finalNewCommands);
	} catch (Exception e) {
	    LOG.error("Fail to edit ContainerRemoteLaunchEvent", e);
	}
    }

    /**
     * All the information are from launchEvent, which gets all the task
     * information from taskAttempt, should match with the task information
     * inside Yarn Child
     * 
     * @param jobID
     * @param taskID
     ...
     * @param rack
     */
    private void recordTaskLocations(String jobID, int taskID, String host, String rack) {
	synchronized (taskLocations) {
	    taskLocations.put(taskID, host);
	    LOG.info("RECORD TASK " + taskID + ", NODE " + host + ". CURRENT NUMBER OF TASKS: " + taskLocations.size());
	    if (taskLocations.size() == numMapTasks && !isPrinted) {
		...
		try {
			...
			for (int i = 0; i < tasks.size(); i++) {
			    ...
			    LOG.info("Node: " + node + ". Task: " + tasks.get(i) + ". Worker: " + workerID);
			    workerID++;
			}
		    }
		    bw1.flush();
		    ...
		    bw2.close();
		    // Write Lock file
		    Path lock = new Path(fs.getHomeDirectory(), lockFile);
