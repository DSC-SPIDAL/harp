...
/*******************************************************
 * Int2ValKVPartition manages key-value pairs in which the key is int-type and
 * the value is object
 ******************************************************/
public abstract class Int2ValKVPartition<V extends Value> extends KVPartition {

    /** Class logger */
    private static final Logger LOG = Logger.getLogger(Int2ValKVPartition.class);
    ...
    /**
     * Clear the partition. All Value objects will be cached in and freeVales
     * for later reuse
     */
    @Override
    public void clear() {
	if (!this.kvMap.isEmpty()) {
	    ObjectIterator<Int2ObjectMap.Entry<V>> iterator = this.kvMap.int2ObjectEntrySet().fastIterator();
	...
	if (this.kvMap == null) {
	    this.kvMap = new Int2ObjectOpenHashMap<>(size);
	    this.kvMap.defaultReturnValue(null);
	}
	try {
