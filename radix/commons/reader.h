...
class Reader {
 ...
 public:
  ...
  EdgeList ReadInBin(std::ifstream &in) {
    EdgeList el;
    SGOffset num_edges;
    in.read(reinterpret_cast<char*>(&num_edges), sizeof(SGOffset));
    pvector<SGEdge> file_el(num_edges);
    std::streamsize file_size_bytes = num_edges * sizeof(SGEdge);
    in.read(reinterpret_cast<char*>(file_el.data()), file_size_bytes);
    el.reserve(num_edges);
    for (SGEdge fe : file_el) {
      el.push_back(Edge(static_cast<NodeID_>(fe.u),
                        static_cast<DestID_>(fe.v)));
    }
    return el;
  }
  ...
  EdgeList ReadInStringEL(std::ifstream &in) {
    EdgeList el;
    std::string orig_u, orig_v;
    NodeID_ next_id = 0;
    std::unordered_map<std::string, NodeID_> renamer;
    while (in >> orig_u >> orig_v) {
      if (renamer.count(orig_u) == 0)
        renamer[orig_u] = next_id++;
      if (renamer.count(orig_v) == 0)
        renamer[orig_v] = next_id++;
      el.push_back(Edge(renamer[orig_u], renamer[orig_v]));
    }
    return el;
  }

  EdgeList ReadHyperStringEL(std::ifstream &in) {
    EdgeList el;
    std::string hyper_edge_str, endpoint_str;
    NodeID_ next_id = 0;
    std::unordered_map<std::string, std::vector<NodeID_>> hyper_edges;
    std::unordered_map<std::string, NodeID_> vertices;
    while (in >> hyper_edge_str >> endpoint_str) {
      if (hyper_edges.count(hyper_edge_str) == 0)
        hyper_edges[hyper_edge_str] = std::vector<NodeID_>();
      if (vertices.count(endpoint_str) == 0)
        vertices[endpoint_str] = next_id++;
      hyper_edges[hyper_edge_str].push_back(vertices[endpoint_str]);
    }
    std::cout << hyper_edges.size() << " hyperedges" << std::endl;
    std::cout << next_id << " vertices" << std::endl;
    uint64_t predicted_edge_count = 0;
    for (auto &kv : hyper_edges) {
      if (kv.second.size() <= 0)
        std::cout << kv.second.size() << std::endl;
      predicted_edge_count += kv.second.size() * (kv.second.size()-1);
    }
    std::cout << predicted_edge_count << " edges predicted" << std::endl;
    std::vector<std::vector<NodeID_>> bins(next_id);
    for (auto &kv : hyper_edges) {
      for (NodeID_ u : kv.second) {
        for (NodeID_ v : kv.second) {
          if (u != v) {
            if (std::find(bins[u].begin(), bins[u].end(), v) == bins[u].end())
              bins[u].push_back(v);
          }
        }
      }
    }
    for (NodeID_ u=0; u < static_cast<NodeID_>(bins.size()); u++) {
      for (NodeID_ v : bins[u]) {
        el.push_back(Edge(u, v));
      }
    }
    return el;
  ...
  EdgeList ReadFile(bool &needs_weights) {
    ...
    } else if (suffix == ".strel") {
      el = ReadInStringEL(file);
    } else if (suffix == ".hstrel") {
      el = ReadHyperStringEL(file);
    } else {
  ...
  RCSRGraph<NodeID_, DestID_, invert> ReadSerializedGraph() {
    ...
    if (directed && invert) {
      ...
      inv_index = RCSRGraph<NodeID_, DestID_>::GenIndex(offsets, inv_neighs);
    }
    file.close();
    t.Stop();
    PrintTime("Read Time", t.Seconds());
    if (directed)
      return RCSRGraph<NodeID_, DestID_, invert>(num_nodes, index, neighs,
