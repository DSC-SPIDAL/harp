...
public:
     ...
     * \param[out] stat         Status of the RowMergedNumericTable construction
     */
    static services::SharedPtr<RowMergedNumericTable> create(const NumericTablePtr &nestedTable,
    ...
    services::Status addNumericTable(NumericTablePtr table)
    {
        ...
        if (ncols == 0)
        {
            ...
            _ddict = NumericTableDictionaryPtr(new NumericTableDictionary(ncols, featuresEqual));
            services::Status s = setNumberOfColumnsImpl(cols);
            if(!s)
                return s;
