...
public class EventCollective {
    ...
    /** Class logger */
    protected static final Logger LOG = Logger.getLogger(EventCollective.class);
	...
	try {
		...
		if (destID != workers.getMasterID()) {
		    if (workers.isMaster()) {
			DoubleArray doubleArray = BcastCollective.generateDoubleArray(100);
			client.submitMessageEvent(new Event(EventType.MESSAGE_EVENT, contextName, workers.getSelfID(),
				destID, doubleArray));
		    } else {
		...
		if (workers.isMaster()) {
		    DoubleArray doubleArray = BcastCollective.generateDoubleArray(100);
		    client.submitCollectiveEvent(new Event(EventType.COLLECTIVE_EVENT, contextName, workers.getSelfID(),
	    ...
	    // Receive event
	    LOG.info("Start receiving. Event count: " + eventCount);
	    for (int i = 0; i < eventCount; i++) {
		Event event = eventQueue.waitEvent();
		DoubleArray doubleArray = (DoubleArray) event.getBody();
		int start = doubleArray.start();
		int size = doubleArray.size();
		LOG.info("Receive event: " + event.getEventType() + ". Double Array: First double: "
			+ doubleArray.get()[start] + ", last double: " + doubleArray.get()[start + size - 1]);
		doubleArray.release();
	    }
	} catch (Exception e) {
	    LOG.error("Error in send/receive events.", e);
	}
	// -------------------------------------------------------
