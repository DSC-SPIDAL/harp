...
public class AllgatherCollective {
    ...
    private static final Logger LOG = Logger.getLogger(AllgatherCollective.class);
	...
	for (int i = 0; i < numPartitions; i++) {
	    ...
	    // workerID
	    Partition<DoubleArray> partition = new Partition<DoubleArray>(workerID * numPartitions + i, doubleArray);
     ...
     */
    public static <P extends Simple> boolean allgather(final String contextName, final String operationName,
	...
	DataSender sender = new DataSender(sendData, nextID, workers, Constant.SEND_DECODE);
	sender.execute();
	// Release
	...
	LinkedList<Data> recvDataList = new LinkedList<>();
	...
	if (isFailed) {
	    ...
	    PartitionUtil.addPartitionsToTable(partitions, table);
	}
	return !isFailed;
    }
}
