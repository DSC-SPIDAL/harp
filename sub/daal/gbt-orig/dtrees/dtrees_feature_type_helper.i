    ...
    IndexedFeatures::FeatureEntry& entry, IndexType* aRes, size_t iCol, size_t nRows, bool bUnorderedFeature)
{
    ...
    for(; i + binSize < nRows;)
    {
        ...
        {
            //right border cant be placed at iRight because it has to be between different feature values
    ...
    const BinParams* pBimPrm)
{
    ...
    if(0){
        ...
        for(size_t i = 0; i < nC; i++){ 
            if (_entries[i].numIndices == 2){
                colcnt ++;
                //2 value columns
                IndexType* pData = _data + _nRows*i;
                size_t k = 0;
        ...
        std::cout << "nR:" << nR << " nC:" << nC << " colcnt:" << colcnt << " truecnt:" << truevalcnt << 
            " maxK:" << maxK << "\n";
