...
// the skeleton of the distri xgboost
namespace xgboost {
 ...
 * */
        ...
        if (param.silent == 0) {
            ...
            for (int i = 0; i < 10; i++) {
                LOG(CONSOLE) << "Aux Time " << i << ": " << std::fixed << tminfo.aux_time[i];
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    hgbtWorker.init(argc, argv);
    // start the mpi procs underyling harp
    hgbtWorker.start(argc, argv);
    return 0;
}
