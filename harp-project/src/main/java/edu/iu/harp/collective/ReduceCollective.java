...
public class ReduceCollective {
    ...
    private static final Logger LOG = Logger.getLogger(ReduceCollective.class);
	...
	for (int i = 0; i < numPartitions; i++) {
	    ...
	    LOG.info("Data Generate, WorkerID: " + workerID + " Partition: " + partition.id() + " Row count: "
		    + doubles[0] + " First element: " + doubles[1] + " Last element: " + doubles[doublesSize - 1]);
	...
	for (Partition<DoubleArray> partition : table.getPartitions()) {
	    ...
	    LOG.info(" Partition: " + partition.id() + " Row count: " + doubles[0] + " First element: " + doubles[1]
     ...
     */
    public static <P extends Simple> boolean reduce(final String contextName, final String operationName,
	    final Table<P> table, final int reduceWorkerID, final DataMap dataMap, final Workers workers) {
	...
	Int2ObjectOpenHashMap<Data> cachedDataMap = new Int2ObjectOpenHashMap<>();
	while (left < right) {
	    if (selfID <= middle && reduceWorkerID > middle) {
		destID = selfID + half;
		isSend = true;
		isRecv = false;
	    ...
	    } else if (selfID <= middle && reduceWorkerID <= middle) {
		...
		Data sendData = new Data(DataType.PARTITION_LIST, contextName, selfID, ownedPartitions,
		...
		{
		    Data recvData = cachedDataMap.remove(sourceID);
		    // Wait data
		    if (recvData == null) {
			while (true) {
			    recvData = IOUtil.waitAndGet(dataMap, contextName, operationName);
			    if (recvData == null) {
				isFailed = true;
				break;
			    } else {
				...
				if (recvData.getWorkerID() != sourceID) {
				    cachedDataMap.put(recvData.getWorkerID(), recvData);
				} else {
				    break;
				}
			    }
		...
		if (range % 2 == 1 && selfID == (middle + 1) && !isFailed) {
		    // LOG.info("Get extra data from middle: "
		    // + middle);
		    Data extraData = cachedDataMap.remove(middle);
		    // Wait data
		    if (extraData == null) {
			while (true) {
			    extraData = IOUtil.waitAndGet(dataMap, contextName, operationName);
			    if (extraData == null) {
				isFailed = true;
				break;
			    } else {
				...
				if (extraData.getWorkerID() != middle) {
				    cachedDataMap.put(extraData.getWorkerID(), extraData);
				} else {
				    break;
				}
			    }
		    ...
		    if (!isFailed) {
			PartitionUtil.addPartitionsToTable(extraData.getBody(), table);
		    }
		}
		if (isFailed) {
