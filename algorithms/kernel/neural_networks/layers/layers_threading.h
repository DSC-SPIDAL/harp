...
template<CpuType cpu, typename F>
Status computeImplStatus(const Tensor &inputTensor, const F &processBlock, const size_t minElementsNumInBlock = 997)
{
    ...
    else
    {
        ...
        daal::threader_for(nBlocks, nBlocks, [ =, &safeStat, &dims ](size_t i)
        {
            ...
            getFixedDimsIndexes(fDimN, fDims, dims, i);
            DAAL_CHECK_STATUS_THR(processBlock(fDimN, fDims, dims[fDimN], inputLayout));
        } );
