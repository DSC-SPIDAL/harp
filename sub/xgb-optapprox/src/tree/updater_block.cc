 ...
 * \file updater_HistMakerBlockSparse.cc
...
class HistMakerBlockSparse: public BaseMaker {
 public:

 HistMakerBlockSparse(){

      this->isInitializedHistIndex = false;
      p_blkmat = new DMatrixCube<unsigned short>();
 ...
 protected:
  ...
  struct ThreadWSpace {
    ...
    void saveGHSumCompact(int treeid, int depth, int nodecnt){
      std::ostringstream ss;
      ss << "ghsum_" << treeid << "_" << depth;
  ...
  struct HistEntry {
    typename HistMakerBlockSparse<TStats>::HistUnit hist;
    //unsigned istart;
   ...
   */
    class BuildHistTask: public tbb::task {
        TreeNode* root;
        HistMakerBlockSparse* ctx;
        bool is_continuation;

    public:
        BuildHistTask( TreeNode* root_, HistMakerBlockSparse* ctx_ ) : 
 ...
 // hist mat compact
  DMatrixCube<unsigned short>* p_blkmat;
 ...
 */
  ...
  void UpdateHistBlockAll(const std::vector<GradientPair> &gpair,
                            const DMatrixCubeZCol<unsigned short> &zcol,
                            const MetaInfo &info,
                            const RegTree &tree,
  ...
  void UpdateHistBlock(const std::vector<GradientPair> &gpair,
                            const DMatrixCubeBlock<unsigned short> &block,
                            const MetaInfo &info,
                            const RegTree &tree,
                            bst_uint blkid_offset,
                            int zblkid,
...
XGBOOST_REGISTER_TREE_UPDATER(HistMakerBlockSparse, "grow_block")
.describe("Tree constructor that uses approximate global of histogram construction.")
.set_body([]() {
    return new HistMakerBlockSparse<GradStats>();
  });
}  // namespace tree
}  // namespace xgboost
