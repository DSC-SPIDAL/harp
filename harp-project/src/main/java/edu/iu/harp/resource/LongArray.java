 ...
 ******************************************************/
public final class LongArray extends Array<long[]> {
     ...
     * @return
     */
    public static LongArray create(int len, boolean approximate) {
	if (len > 0) {
	    long[] longs = ResourcePool.get().getLongsPool().getArray(len, approximate);
	    if (longs != null) {
    ...
    @Override
    public void release() {
	ResourcePool.get().getLongsPool().releaseArray(array);
	this.reset();
    }
    ...
    @Override
    public void free() {
	ResourcePool.get().getLongsPool().freeArray(array);
	this.reset();
    }
}
