...
services::Status DAAL_EXPORT Input::allocate(const daal::algorithms::Parameter *parameter, const int method)
{
    ...
    {
        TensorPtr tensor(new HomogenTensor<algorithmFPType>(getWeightsSizes(param), Tensor::doAllocate));
        set(layers::forward::weights, tensor);
    }

    if( !get(layers::forward::biases) )
    {
        TensorPtr tensor(new HomogenTensor<algorithmFPType>(getBiasesSizes(param), Tensor::doAllocate));
