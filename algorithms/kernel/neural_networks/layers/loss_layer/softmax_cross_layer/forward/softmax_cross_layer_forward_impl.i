    ...
    Tensor &resultTensor)
{
    ...
    daal::threader_for(nBlocks, nBlocks, [ =, &blockLoss, &safeStat, &inputTensor, &groundTruthTensor, &probabilitiesTensor ](int block)
    {
        ...
        DAAL_CHECK_STATUS_THR(processBlock(inputTensor, groundTruthTensor, block * _nRowsInBlock, nRowsToProcess, probabilitiesTensor, dim, eps, safeStat, *partialLoss));
    }
                      );
    ...
    algorithmFPType &partialLoss)
{
...

    {
        ...
        HomogenTensor<algorithmFPType> softmaxInput(softmaxDim, const_cast<algorithmFPType *>(inputArray));
        HomogenTensor<algorithmFPType> softmaxProb(softmaxDim, probArray);
