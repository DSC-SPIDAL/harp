        ...
        NumericTable *batchIndices, OptionalArgument *optionalArgument, OptionalArgument *optionalResult)
{
    ...
    for(size_t epoch = task.startIteration; epoch < (task.startIteration + nIter); epoch++)
    {
        if(task.indicesStatus == user || task.indicesStatus == random)
        {
            ...
            DAAL_CHECK_STATUS(s, rngTask.get(pValues, RngTask<int, cpu>::eUniformWithoutReplacement));
    ...
    if(parameter->optionalResultRequired)
        s |= rngTask.save(optionalResult, sgd::rngState);
    return s;
}
    ...
    NumericTable *lastIterationInput)
{
    ...
    {
        ntBatchIndices.reset(new HomogenNumericTableCPU<int, cpu>(NULL, batchSize, 1));
    ...
    if(!parameter->optionalResultRequired)
    {
        SharedPtr<HomogenNumericTableCPU<algorithmFPType, cpu>> pastUpdateCpuNt(new HomogenNumericTableCPU<algorithmFPType, cpu>(1, argumentSize));
