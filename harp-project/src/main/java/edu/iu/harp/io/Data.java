...
/*******************************************************
 * The wrapper of data used in communication. The fields in data must be
 * consistent
 ******************************************************/
public class Data {
    ...
    private byte bodyType = DataType.UNKNOWN_DATA_TYPE;
    /** This usually is the name of event handler */
    private String contextName = null;
    /** Source worker ID */
    private int workerID = Constant.UNKNOWN_WORKER_ID;
    /** Only required in collective communication */
    private String operationName = null;
    /** Only required in collective communication */
    private int partitionID = Constant.UNKNOWN_PARTITION_ID;
    ...
    /** Construct a data */
    public Data(byte type, String conName, int wID, List<Transferable> object, int s) {
	bodyType = type;
	contextName = conName;
	workerID = wID;
	bodySize = s;
    ...
    public Data(byte type, String conName, int wID, List<Transferable> object, int s, String opName) {
	bodyType = type;
	contextName = conName;
	workerID = wID;
    ...
    public Data(byte type, String conName, int wID, List<Transferable> object, int s, String opName, int parID) {
	bodyType = type;
	contextName = conName;
	workerID = wID;
	operationName = opName;
	...
	if (headArray == null || headArray.get() == null) {
	    headArray = null;
	    headStatus = DataStatus.DECODE_FAILED;
	}
	bodyArray = null;
	...
	if (headArray == null || headArray.get() == null || bodyArray == null || bodyArray.get() == null) {
	    headArray = null;
	    headStatus = DataStatus.DECODE_FAILED;
	    bodyArray = null;
	    bodyStatus = DataStatus.DECODE_FAILED;
	...
	if (bodyType == DataType.UNKNOWN_DATA_TYPE || contextName == null || bodySize == 0
    ...
    /**
     * Release the head array. If the array is removed without encode/decode,
     * make sure that the data status is correct. Other states shouldn't have
     * encoded head array
     */
    ...
    /**
     * Release the data body array. If the array is removed without
     * encode/decode, make sure that the data status is correct
     */
	...
	if (bodyStatus == DataStatus.DECODED || bodyStatus == DataStatus.ENCODE_FAILED_DECODED) {
	    releaseBody(body);
	}
	resetData();
	headStatus = DataStatus.DECODE_FAILED;
     ...
     *            the body
     */
    private void releaseBody(List<Transferable> trans) {
	...
	if (headStatus == DataStatus.ENCODED_ARRAY) {
	    ...
	    // be encoded array
	    Deserializer deserializer = new Deserializer(headArray);
	    ...
	    if (!isFailed && (deserializer.getPos() < deserializer.getLength())) {
		try {
		    operationName = deserializer.readUTF();
		} catch (IOException e) {
		    LOG.error("Fail to decode operation name", e);
		    resetHead();
		    isFailed = true;
		}
	    }
		...
		try {
		    ...
		    LOG.error("Fail to decode partition ID", e);
		    resetHead();
		    isFailed = true;
		}
	    }
	    if (isFailed) {
		headStatus = DataStatus.ENCODED_ARRAY_DECODE_FAILED;
	    } else {
		headStatus = DataStatus.ENCODED_ARRAY_DECODED;
		if (bodyArray == null && bodyStatus == DataStatus.DECODE_FAILED) {
		    // Prepare body array
		    // if there is no such one
		    bodyArray = ByteArray.create(bodySize, true);
		    if (bodyArray != null) {
			bodyStatus = DataStatus.ENCODED_ARRAY;
		    }
		}
	...
	if ((headStatus == DataStatus.DECODED || headStatus == DataStatus.ENCODED_ARRAY_DECODED
		|| headStatus == DataStatus.ENCODE_FAILED_DECODED) && bodyStatus == DataStatus.ENCODED_ARRAY) {
	    // If body status is encoded array
	    // body array cannot be null.
	    // body object must be null;
	    if (bodyType == DataType.SIMPLE_LIST) {
		body = DataUtil.decodeSimpleList(bodyArray);
	    } else if (bodyType == DataType.PARTITION_LIST) {
		body = DataUtil.decodePartitionList(bodyArray);
	    } else {
		LOG.error("Cannot decode unknown body: " + bodyType);
	    }
	    if (body == null) {
		bodyStatus = DataStatus.ENCODED_ARRAY_DECODE_FAILED;
	    } else {
		bodyStatus = DataStatus.ENCODED_ARRAY_DECODED;
	    }
	}
	return bodyStatus;
    }
	...
	if (headStatus == DataStatus.DECODED) {
	    ...
	    int headArrSize = 9 + (4 + 2 * contextName.length());
	    if (isOpData) {
		headArrSize += (4 + 2 * operationName.length());
	    }
	    if (isParData) {
		headArrSize += 4;
	    }
	    headArray = ByteArray.create(headArrSize, true);
	    Serializer serializer = new Serializer(headArray);
	    boolean isFailed = false;
	    try {
		...
		LOG.error("Fail to encode body type, context name" + "and worker ID.", e);
		isFailed = true;
	    }
		...
		try {
		    ...
		    LOG.error("Fail to encode operation name.", e);
		    isFailed = true;
		}
	    }
		...
		try {
		    ...
		    LOG.error("Fail to encode parititon ID.", e);
		    isFailed = true;
		}
	    }
	    if (isFailed) {
	...
	if (bodyStatus == DataStatus.DECODED) {
		...
		int size = DataUtil.getNumTransListBytes(body);
		bodyArray = ByteArray.create(size, true);
	    }
	    DataOutput dataOut = new Serializer(bodyArray);
	    if (bodyType == DataType.SIMPLE_LIST || bodyType == DataType.PARTITION_LIST) {
		try {
		    DataUtil.encodeTransList(body, bodySize, dataOut);
		} catch (Exception e) {
		    bodyArray.release();
		    bodyArray = null;
		}
	    } else {
		LOG.info("Cannot encode unknown data type.");
	    }
	    if (bodyArray != null) {
		bodyStatus = DataStatus.ENCODED_ARRAY_DECODED;
	    } else {
