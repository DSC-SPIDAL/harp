...
/*******************************************************
 * Int2ValKVPartitionCombiner defines how to merge Int2ValKVPartition
 ******************************************************/
class Int2ValKVPartitionCombiner<V extends Value, P extends Int2ValKVPartition<V>> extends PartitionCombiner<P> {

    private ValCombiner<V> valCombiner;

    Int2ValKVPartitionCombiner(ValCombiner<V> combiner) {
	...
	ObjectIterator<Int2ObjectMap.Entry<V>> iterator = nMap.int2ObjectEntrySet().fastIterator();
	IntArrayList rmKeys = new IntArrayList();
	while (iterator.hasNext()) {
	    Int2ObjectMap.Entry<V> entry = iterator.next();
	    int key = entry.getIntKey();
	    ValStatus status = op.putKeyVal(key, entry.getValue(), valCombiner);
	    if (status == ValStatus.ADDED) {
		rmKeys.add(key);
		if (!op.getFreeVals().isEmpty()) {
		    np.getFreeVals().add(op.getFreeVals().remove(0));
		}
	    }
	}
...
public abstract class Int2ValKVTable<V extends Value, P extends Int2ValKVPartition<V>> extends KVTable<P> {
    ...
    private static final Logger LOG = Logger.getLogger(Int2ValKVTable.class);

    private final Class<V> vClass;
    private final Class<P> pClass;
    private final ValCombiner<V> combiner;

    public Int2ValKVTable(int tableID, ValCombiner<V> combiner, Class<V> vClass, Class<P> pClass) {
	super(tableID, new Int2ValKVPartitionCombiner<V, P>(combiner));
	this.vClass = vClass;
	this.pClass = pClass;
     ...
     */
    private ValStatus addKVInPartition(P partition, int key, V val) {
	return partition.putKeyVal(key, val, combiner);
    }
	...
	Partition<P> partition = this.getPartition(partitionID);
	if (partition == null) {
	    partition = new Partition<>(partitionID, Writable.create(pClass));
