...
public class MLRMapCollective extends Configured
  implements Tool {

  public static void main(String[] argv)
    throws Exception {
    int res = ToolRunner.run(new Configuration(),
      new MLRMapCollective(), argv);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
    if (args.length < 9) {
      System.err.println(
        "Usage: MLRMapCollective <alpha> <#iter> <#terms> "
    ...
    // Configure jobs
    Job job = configureMLRJob(alpha, ITER, TERM,
      numMapTask, numThread, topicsPath,
      qrelsPath, trainPath, outputPath,
      configuration);
  ...
  private Job configureMLRJob(double alpha,
    int ITER, int TERM, int numMapTask,
    int numThread, String topicPath,
    String qrelsPath, String dataPath,
    String outputPath,
    Configuration configuration)
    throws IOException, URISyntaxException {
    Job job =
      Job.getInstance(configuration, "MLR_job");
    JobConf jobConf =
