...
public class Acceptor implements Runnable {
    ...
    public Acceptor(ServerConn conn, EventQueue queue, DataMap map, Workers w, byte command) {
	this.conn = conn;
    ...
    /**
     * The overridden run function for handling the messages received from a
     * client
     */
    @Override
    public void run() {
	...
	try {
		...
		if (commandType == Constant.CONNECTION_END) {
		    ...
		    DataReceiver receiver = new DataReceiver(selfID, conn, eventQueue, dataMap, Constant.SEND);
		    receiver.run();
		} else if (commandType == Constant.SEND_DECODE) {
		    DataReceiver receiver = new DataReceiver(selfID, conn, eventQueue, dataMap, Constant.SEND_DECODE);
		    receiver.run();
		} else if (commandType == Constant.CHAIN_BCAST) {
		    Receiver receiver = new DataChainBcastReceiver(selfID, conn, eventQueue, dataMap, workers,
		    ...
		    Receiver receiver = new DataMSTBcastReceiver(selfID, conn, eventQueue, dataMap, workers,
			    Constant.MST_BCAST);
		    receiver.run();
		    ...
		    LOG.info("Unknown command: " + commandType);
		    break;
		}
