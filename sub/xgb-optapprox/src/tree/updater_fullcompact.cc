 ...
 * \file updater_HistMakerFullCompact.cc
...
DMLC_REGISTRY_FILE_TAG(updater_fullcompact);

template<typename TStats>
class HistMakerFullCompact: public BaseMaker {
 public:
...
template<typename TStats>
class CQHistMakerFullCompact: public HistMakerFullCompact<TStats> {
 public:
  CQHistMakerFullCompact()  = default;

 protected:
  struct HistEntry {
    typename HistMakerFullCompact<TStats>::HistUnit hist;
    unsigned istart;
...
// global proposal
template<typename TStats>
class GlobalProposalHistMakerFullCompact: public CQHistMakerFullCompact<TStats> {

 public:
  GlobalProposalHistMakerFullCompact(){
 ...
 protected:
  void ResetPosAndPropose(const std::vector<GradientPair> &gpair,
      ...
      if(!this->isInitializedHistIndex){
        ...
        CQHistMakerFullCompact<TStats>::ResetPosAndPropose(gpair, p_fmat, fset, tree);
        LOG(CONSOLE) << "ResetPosAndPropose call in globalproposal";

        //this->wspace_.Init(this->param_, 1, 1);
        //CQHistMakerFullCompact<TStats>::InitHistIndex(p_fmat, fset, tree);
...
XGBOOST_REGISTER_TREE_UPDATER(HistMakerFullCompact, "grow_fullcompact")
.describe("Tree constructor that uses approximate global of histogram construction.")
.set_body([]() {
    return new GlobalProposalHistMakerFullCompact<GradStats>();
  });
}  // namespace tree
}  // namespace xgboost
