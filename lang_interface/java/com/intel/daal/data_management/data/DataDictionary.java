...
public class DataDictionary extends SerializableBase {
    ...
    static public class FeaturesEqual implements Serializable {
        private static final int _notEqual = 0;
        private static final int _equal    = 1;

        public static final FeaturesEqual notEqual = new FeaturesEqual(_notEqual);
        public static final FeaturesEqual equal    = new FeaturesEqual(_equal);

        private final int _value;

        FeaturesEqual(final int value) {
            this._value = value;
        }
     ...
     */
    public DataDictionary(DaalContext context, long nFeatures, FeaturesEqual featuresEqual) {
        super(context);
        _dictionaryAllocatedInJava = true;
        _featuresEqual = featuresEqual;
        this.cObject = init((int) nFeatures, _featuresEqual.ordinal());
    ...
    public DataDictionary(DaalContext context, long nFeatures) {
        super(context);
        _dictionaryAllocatedInJava = true;
        _featuresEqual = FeaturesEqual.notEqual;
        this.cObject = init((int) nFeatures, _featuresEqual.ordinal());
        dict = new Vector<DataFeature>();
        setNumberOfFeatures(nFeatures);
    }
    ...
    public DataDictionary(DaalContext context, long nFeatures, long cDictionary) {
        ...
        _featuresEqual = new FeaturesEqual(cGetCDataDictionaryFeaturesEqual(cObject));
        dict = new Vector<DataFeature>();
        if (_featuresEqual.ordinal() == FeaturesEqual.equal.ordinal()) {
            dict.setSize(1);
        } else {
     ...
     */
    public FeaturesEqual getFeaturesEqual() {
        return _featuresEqual;
    }

    /* Constructs C++ data dictionary object */
    private native long init(int nFeatures, int featuresEqual);
    ...
    private native int cGetCDataDictionaryFeaturesEqual(long cObject);
    ...
    protected void onUnpack(DaalContext context) {
        ...
        if (_dictionaryAllocatedInJava) {
            cObject = init((int) serializedNFeatures, _featuresEqual.ordinal());
        }
    }
    ...
    private FeaturesEqual  _featuresEqual;
