...
DAAL_EXPORT services::Status Input::allocate(const daal::algorithms::Parameter *parameter, const int method)
{
    ...
    {
        SharedPtr<Tensor> tensor(new HomogenTensor<algorithmFPType>(getWeightsSizes(param), Tensor::doAllocate));
        set(layers::forward::weights, tensor);
    }

    if( !get(layers::forward::biases) )
    {
        SharedPtr<Tensor> tensor(new HomogenTensor<algorithmFPType>(getBiasesSizes(param), Tensor::doAllocate));
