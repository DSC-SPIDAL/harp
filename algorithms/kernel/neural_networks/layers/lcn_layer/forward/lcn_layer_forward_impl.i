...
Status LCNKernel<algorithmFPType, method, cpu>::calculateCenteredDataAndSigma(const lcn::Parameter &parameter, const Tensor &inputTensor)
{
    ...
    TensorPtr weightsTensor = TensorPtr(new HomogenTensor<algorithmFPType>(weightsDims, weightsArray));
    DAAL_CHECK_MALLOC(weightsTensor);
    ...
    TensorPtr convInputTensor = TensorPtr(new HomogenTensor<algorithmFPType>(dataDims, const_cast<algorithmFPType *>(inputArray)));
    DAAL_CHECK_MALLOC(convInputTensor);

    TensorPtr biasesTensor = TensorPtr(new HomogenTensor<algorithmFPType>(bDims, biasesArray));
    DAAL_CHECK_MALLOC(biasesTensor);

    TensorPtr valueTensor  = TensorPtr(new HomogenTensor<algorithmFPType>(sDims, const_cast<algorithmFPType *>(sigmaArray)));
    DAAL_CHECK_MALLOC(valueTensor);
    ...
    convInputTensor = TensorPtr(new HomogenTensor<algorithmFPType>(dataDims, const_cast<algorithmFPType *>(resultArray)));
    DAAL_CHECK_MALLOC(convInputTensor);
