...
public class Depl {
    ...
    private static final Logger LOG = Logger.getLogger(Depl.class);
    ...
    public final static String nodes_file = prjt_home + bin_directory + "nodes";
	...
	List<String> output = cmdOutput.getExeOutput();
	try {
	...
	Output cmdOutput = Depl.executeCMDandReturn(cmdstr);
	String java_home = null;
	if (cmdOutput.getExeStatus()) {
	    // Home directory is returned with "/" at
	    // the end
	    java_home = cmdOutput.getExeOutput().get(0) + "/";
	...
	if (cmdOutput.getExeStatus()) {
	    // Home directory is returned with "/" at
	    // the end
	    harp_home = cmdOutput.getExeOutput().get(0) + "/";
	...
	if (Depl.getJavaHomePath() == null) {
	    ...
	    File javaHomeFile = new File(Depl.getJavaHomePath());
	...
	if (invalidJavaHome) {
	    ...
	    LOG.info("Java Home: " + Depl.getJavaHomePath());
	}
	// Project home detection
	if (Depl.getProjectHomePath() == null) {
	    invalidProjectHome = true;
	} else {
	    File homeFile = new File(Depl.getProjectHomePath());
	    if (!homeFile.exists()) {
		invalidProjectHome = true;
	    }
	}
	if (invalidProjectHome) {
	    ...
	    LOG.info("Harp Home: " + Depl.getProjectHomePath());
	}
	...
	if (invalidNodes) {
	    LOG.error("No or wrong nodes file format is provided.");
	    System.exit(-1);
	}
	...
	nodes.sortRacks();
	configureStatus = writeToFile(Depl.nodes_file, nodes.printToNodesFile());
	if (!configureStatus) {
	    LOG.error(" Errors when configuring nodes.");
	    System.exit(-1);
	} else {
