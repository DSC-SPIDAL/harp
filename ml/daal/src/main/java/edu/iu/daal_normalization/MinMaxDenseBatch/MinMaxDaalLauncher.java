...
package edu.iu.daal_normalization.MinMaxDenseBatch;
...
public class MinMaxDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 8) {
      System.err
        .println("Usage: edu.iu.daal_svd.MinMaxDaalLauncher "
          + "<file dim > "
	  + "<lower bound>"
	  + "<upper bound>"
	  + "<number of map tasks>"
    ...
    launch(nFileDim, lowerbound, upperbound, numMapTasks, numThreads, mem, trainDataDir, workDir);

    return 0;
  }

  private void launch(int nFileDim, double lowerbound, double upperbound, 
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runMinMax(nFileDim, lowerbound, upperbound, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total MinMax Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runMinMax(
    int nFileDim, double lowerbound, double upperbound, 
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job MinMaxJob =
      configureMinMaxJob(nFileDim, lowerbound, upperbound, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      MinMaxJob.waitForCompletion(true);
    System.out
      .println("end Jod "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("MinMax Job fails.");
    }
  }

  private Job configureMinMaxJob(
    int nFileDim, double lowerbound, double upperbound, 
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "MinMax_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(MinMaxDaalLauncher.class);
    job
      .setMapperClass(MinMaxDaalCollectiveMapper.class);
    ...
    jobConfig.setInt(Constants.NUM_THREADS,
      numThreads);
    jobConfig.setDouble(Constants.LOWER_BOUND,
      lowerbound);
    jobConfig.setDouble(Constants.UPPER_BOUND,
      upperbound);
