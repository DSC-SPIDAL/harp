...
ModelNormEqInternal::ModelNormEqInternal(size_t featnum, size_t nrhs, const linear_regression::Parameter &par, modelFPType dummy):
    super(featnum, nrhs, par, dummy)
{
    ...
    _xtxTable.reset(new HomogenNumericTable<modelFPType>(dimWithoutBeta, dimWithoutBeta, NumericTable::doAllocate, 0));
    _xtyTable.reset(new HomogenNumericTable<modelFPType>(dimWithoutBeta, nrhs, NumericTable::doAllocate, 0));
}
