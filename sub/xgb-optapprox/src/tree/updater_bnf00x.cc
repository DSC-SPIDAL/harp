 ...
 * \file updater_HistMakerBNF00X.cc
...
DMLC_REGISTRY_FILE_TAG(updater_bnf00x);

template<typename TStats>
class HistMakerBNF00X: public BaseMaker {
 public:
 ...
 protected:
  ...
  struct HistEntry {
    typename HistMakerBNF00X<TStats>::HistUnit hist;
    //unsigned istart;
   ...
   */
    ...
    public:
        BuildHistTask( TreeNode* root_, HistMakerBNF00X* ctx_ ) : 
...
XGBOOST_REGISTER_TREE_UPDATER(HistMakerBNF00X, "grow_bnf00x")
.describe("Tree constructor that uses approximate global of histogram construction.")
.set_body([]() {
    return new HistMakerBNF00X<GradStats>();
  });
}  // namespace tree
}  // namespace xgboost
