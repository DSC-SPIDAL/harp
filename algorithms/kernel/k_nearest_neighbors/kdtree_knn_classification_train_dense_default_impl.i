                 ...
                 compute(NumericTable * x, NumericTable * y, kdtree_knn_classification::Model * r, const daal::algorithms::Parameter * par)
{
    ...
    r->impl()->setKDTreeTable(KDTreeTablePtr(new KDTreeTable(maxKDTreeNodeCount)));
    ...
    DAAL_CHECK_STATUS(status, buildFirstPartOfKDTree(q, bboxQ, *x, *r, indexes, parameter->seed));
    DAAL_CHECK_STATUS(status, buildSecondPartOfKDTree(q, bboxQ, *x, *r, indexes, parameter->seed));
...
algorithmFpType KNNClassificationTrainBatchKernel<algorithmFpType, training::defaultDense, cpu>::
    computeApproximatedMedianInParallel(size_t start, size_t end, size_t dimension, algorithmFpType upper, const NumericTable & x,
                                        const size_t * indexes, int seed, algorithmFpType * subSamples, size_t subSampleCapacity, Status &status)
{
                            ...
                            kdtree_knn_classification::Model & r, size_t * indexes, int seed)
{
    ...
    daal::threader_for(blockCount, blockCount, [=, &localTLS, &firstNodeIndex, &kdTreeTable, &x, &r, &rowsPerBlock, &xColumnCount, &safeStat](int iBlock)
    {
    ...
    if (isNeedToReindex)
    {
        ...
        KDTreeTablePtr newKDTreeTable(new KDTreeTable(actualNodeCount));
