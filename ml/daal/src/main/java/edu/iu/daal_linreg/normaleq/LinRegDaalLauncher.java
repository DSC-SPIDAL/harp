...
implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (args.length < 8) {
      System.err.println("Usage: edu.iu.daal_linreg"
                + "<input train dir> "
                + "<input test dir>"
...
ExecutionException, ClassNotFoundException {
  ...
  runLinReg(inputDir, testDirPath, testGroundTruthDirPath, mem, batchSize, numMapTasks,
      numThreadsPerWorker, modelDir,
      outputDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total Linear Regression Execution Time: "
        + (endTime - startTime));
}
...
private void runLinReg(Path inputDir, String testDirPath, String testGroundTruthDirPath, int mem, int batchSize,  
    ...
    Job linRegJob =
      configureLinRegJob(inputDir, testDirPath, testGroundTruthDirPath, mem, batchSize,
        numMapTasks, numThreadsPerWorker,
        modelDir, outputDir, configuration);

    boolean jobSuccess =
      linRegJob.waitForCompletion(true);
  ...
  private Job configureLinRegJob(Path inputDir,
