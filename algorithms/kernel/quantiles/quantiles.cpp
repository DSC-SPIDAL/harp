...
Status Input::check(const daal::algorithms::Parameter *parameter, int method) const
{
    ...
    Status s = data_management::checkNumericTable(algParameter->quantileOrders.get(), quantileOrdersStr(), 0, 0, 0, 1);

    s |= data_management::checkNumericTable(get(data).get(), dataStr());
    return s;
}
...
Status Result::check(const daal::algorithms::Input *in, const daal::algorithms::Parameter *par, int method) const
{
    ...
    Status s = data_management::checkNumericTable(parameter->quantileOrders.get(), quantileOrdersStr(), 0, 0, 0, 1);
    if(!s) return s;
    ...
    s |= data_management::checkNumericTable(get(quantiles).get(), quantilesStr(), unexpectedLayouts, 0, nFeatures, nVectors);
    return s;
}
