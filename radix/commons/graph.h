...
class RCSRGraph {
 public:
  RCSRGraph() : directed_(false), num_nodes_(-1), num_edges_(-1),
    out_index_(nullptr), out_neighbors_(nullptr),
    in_index_(nullptr), in_neighbors_(nullptr) {}

  RCSRGraph(int64_t num_nodes, DestID_** index, DestID_* neighs) :
  ...
  RCSRGraph(int64_t num_nodes, DestID_** out_index, DestID_* out_neighs,
  ...
  RCSRGraph(RCSRGraph&& other) : directed_(other.directed_),
  ...
  RCSRGraph& operator=(RCSRGraph&& other) {
    if (this != &other) {
      ReleaseResources();
  ...
  void PrintTopology() const {
    for (NodeID_ u : vertices()) {
      std::cout << u << ": ";
      for (DestID_ v : out_neigh(u)) {
        std::cout << v << " ";
      }
      std::cout << std::endl;
    }
  ...
  // for debug
  void PrintTopologySample() const {
    int itr=0;
    for (NodeID_ u : vertices()) {
      if (itr < 10)
      {
