...
public class ODDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 6) {
      System.err
        .println("Usage: edu.iu.daal_svd.ODDaalLauncher "
          + "<file dim > "
	  + "<number of map tasks>"
	  + "<num threads> "
          + "<mem>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runODMultiDense(nFileDim, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total OD Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runODMultiDense(
    int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("OD Job fails.");
    }
  }

  private Job configureODJob(
    int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "OD_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(ODDaalLauncher.class);
    job
      .setMapperClass(ODDaalCollectiveMapper.class);
