...
public class SRTDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 6) {
      System.err
        .println("Usage: edu.iu.daal_svd.SRTDaalLauncher "
          + "<file dim > "
	  + "<number of map tasks>"
	  + "<num threads> "
          + "<mem>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    System.out
      .println("Total SRT Multi Dense Execution Time: "
        + (endTime - startTime));
  }
  ...
  private void runSRT(
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("SRT Job fails.");
    }
  }

  private Job configureSRTJob(
    int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "SRT_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(SRTDaalLauncher.class);
    job
      .setMapperClass(SRTDaalCollectiveMapper.class);
