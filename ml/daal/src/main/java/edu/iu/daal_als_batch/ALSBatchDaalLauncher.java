...
public class ALSBatchDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 7) {
      System.err
        .println("Usage: edu.iu.daal_svd.ALSBatchDaalLauncher "
          + "<file dim > "
	  + "<nFactors>"
	  + "<number of map tasks>"
	  + "<num threads> "
    ...
    long nFactors = Long.parseLong(args[1]);
    ...
    launch(nFileDim, nFactors, numMapTasks, numThreads, mem, trainDataDir, workDir);

    return 0;
  }

  private void launch(int nFileDim, long nFactors,  
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runALSBatch(nFileDim, nFactors,  
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total ALSBatch Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runALSBatch(
    int nFileDim, long nFactors,  
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job ALSBatchJob =
      configureALSBatchJob(nFileDim, nFactors,  
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      ALSBatchJob.waitForCompletion(true);
    System.out
      .println("end Jod "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("ALSBatch Job fails.");
    }
  }

  private Job configureALSBatchJob(
    int nFileDim, long nFactors,  
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "ALSBatch_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(ALSBatchDaalLauncher.class);
    job
      .setMapperClass(ALSBatchDaalCollectiveMapper.class);
    ...
    jobConfig.setInt(Constants.NUM_THREADS,
      numThreads);
    jobConfig.setLong(Constants.NUM_FACTOR,
      nFactors);
