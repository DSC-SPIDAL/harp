...
/*******************************************************
 * Long2DoubleKVPartition manages key-value pairs in which the key is long-type
 * and the value is double-type
 ******************************************************/
public class Long2DoubleKVPartition extends KVPartition {

    private Long2DoubleOpenHashMap kvMap;
    final static double defaultReturnVal = Double.NEGATIVE_INFINITY;
     ...
     */
    public ValStatus putKeyVal(long key, double val, TypeDoubleCombiner combiner) {
	double curVal = kvMap.put(key, val);
	if (curVal == defaultReturnVal) {
	    return ValStatus.ADDED;
	} else {
	    kvMap.put(key, combiner.combine(curVal, val));
	...
	ObjectIterator<Long2DoubleMap.Entry> iterator = kvMap.long2DoubleEntrySet().fastIterator();
