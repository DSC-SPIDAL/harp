...
public class Nodes {
	...
	BufferedReader reader = new BufferedReader(new FileReader(Depl.nodes_file));
	...
	if (reader == null) {
	    LOG.info("Read from default nodes file. " + Depl.nodes_file);
	    reader = new BufferedReader(new FileReader(Depl.nodes_file));
	    initializeNodes(reader);
	    reader.close();
	} else {
	    initializeNodes(reader);
	...
	for (Entry<Integer, List<String>> entry : nodes.entrySet()) {
    ...
    /**
     * Sorting the racks based on the number of the nodes associated with the
     * racks in descending order
     */
    public void sortRacks() {
	SortedMap<Integer, Integer> sortedRacks = new TreeMap<>();
	// Sort racks based on sizes in Natural
	// ordering
	for (Entry<Integer, List<String>> entry : nodes.entrySet()) {
	    sortedRacks.put(entry.getValue().size(), entry.getKey());
	}
	// Put to list
	List<Integer> rackIDs = new LinkedList<>(sortedRacks.values());
	nodeRackIDs.clear();
	...
	List<String> rackNodeList = new LinkedList<String>();
