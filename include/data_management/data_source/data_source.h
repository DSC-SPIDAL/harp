...
protected:
     ...
     */
    template<typename NumericTableType> void allocateNumericTableImpl(services::SharedPtr<NumericTableType> &nt);
     ...
     *  \param   nt      - Pointer to the allocated Numeric Table
     */
    template<typename FPType> void allocateNumericTableImpl(services::SharedPtr<HomogenNumericTable<FPType> > &nt);
    ...
    virtual void setNumericTableDictionary(NumericTablePtr nt)
    {
...
inline void DataSource::allocateNumericTableImpl(services::SharedPtr<NumericTableType> &nt)
{
    nt = services::SharedPtr<NumericTableType>();
}

template<>
inline void DataSource::allocateNumericTableImpl(AOSNumericTablePtr &nt)
{
    size_t nFeatures = _dict->getNumberOfFeatures();
    size_t structureSize = getStructureSize();
    nt = AOSNumericTablePtr(new AOSNumericTable(structureSize, nFeatures, 0));
    setNumericTableDictionary(nt);
}

template<>
inline void DataSource::allocateNumericTableImpl(SOANumericTablePtr &nt)
{
    nt = SOANumericTablePtr();
}

template<typename FPType>
inline void DataSource::allocateNumericTableImpl(services::SharedPtr<HomogenNumericTable<FPType> > &nt)
{
    size_t nFeatures = getNumericTableNumberOfColumns();
    nt = services::SharedPtr<HomogenNumericTable<FPType> >(new HomogenNumericTable<FPType>(nFeatures, 0, NumericTableIface::doNotAllocate));
...
protected:
    ...
    services::Status updateStatistics(size_t ntRowIndex, NumericTable *nt)
    {
        if(!nt)
            return services::Status(services::ErrorNullInputNumericTable);
