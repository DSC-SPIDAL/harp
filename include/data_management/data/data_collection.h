...
public:
    ...
    const services::SharedPtr<T> &get(size_t k)
    {
        return find(k);
    }

    void set(size_t k, const services::SharedPtr<T> &value)
    {
        insert(k) = value;
    }
...
protected:
    services::SharedPtr<T> &insert(size_t k);
    const services::SharedPtr<T> &find(size_t k) const;
