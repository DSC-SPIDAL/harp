 ...
 *            Output value
 ******************************************************/
public class CollectiveMapper<KEYIN, VALUEIN, KEYOUT, VALUEOUT> extends Mapper<KEYIN, VALUEIN, KEYOUT, VALUEOUT> {
    ...
    /**
     * The lock file is used for synchronization. After launching the job, the
     * applicationMaster will write a lock file to the file system if it knows
     * that all the workers are launched correctly. Every launched worker will
     * try to read this lock file. If it exists in the file system, then the
     * worker knows that all other workers are also launched. The workers will
     * also continue to work after all the workers are launched correctly.
     * 
     * @param lockFile
     *            the lock file
     * @param fs
	...
	LOG.info("TRY LOCK FILE " + lockFile + " " + fs.getHomeDirectory());
	Path path = new Path(fs.getHomeDirectory(), lockFile);
	boolean retry = false;
	int retryCount = 0;
	do {
	    try {
		...
		retryCount++;
		LOG.info("Fail to read nodes lock file " + path.toString() + ", retry... Retry count: " + retryCount);
    ...
    /**
     * Load the map from the task file. The map is from task to worker.
     * 
     * @param tasksFile
     *            the tasks file
     * @param fs
	...
	Path path = new Path(fs.getHomeDirectory(), tasksFile);
	try {
		...
		tokens = line.split("\t");
		taskWorkerMap.put(Integer.parseInt(tokens[0]), Integer.parseInt(tokens[1]));
	    }
	    br.close();
	} catch (IOException e) {
    ...
    /**
     * Get the BufferedReader for reading the "nodes" file
     * 
     * @param nodesFile
     * @param fs
     * @return
     * @throws IOException
     */
    private BufferedReader getNodesReader(String nodesFile, FileSystem fs) throws IOException {
	LOG.info("Get nodes file " + nodesFile);
	Path path = new Path(fs.getHomeDirectory(), nodesFile);
	FSDataInputStream in = fs.open(path);
	BufferedReader br = new BufferedReader(new InputStreamReader(in));
	return br;
    }
	...
	try {
	    server = new Server(host, port, eventQueue, dataMap, workers);
	} catch (Exception e) {
	...
	server.start();
	context.getProgress();
	isSuccess = barrier("start-worker", "handshake");
     ...
     * @param contextName
     * @param operationName
     * @return a boolean tells if the operation succeeds
     */
    public boolean barrier(String contextName, String operationName) {
	boolean isSuccess = Communication.barrier(contextName, operationName, dataMap, workers);
     ...
     * @param table
     *            the table used to hold the partitions
     * @param bcastWorkerID
     *            the worker ID of broadcasting data
     * @param useMSTBcast
     *            if minimum-spanning tree algorithm is used
     * @return a boolean tells if the operation succeeds
     */
    public <P extends Simple> boolean broadcast(String contextName, String operationName, Table<P> table,
	    int bcastWorkerID, boolean useMSTBcast) {
	boolean isSucess = BcastCollective.broadcast(contextName, operationName, table, bcastWorkerID, useMSTBcast,
		dataMap, workers);
	dataMap.cleanOperationData(contextName, operationName);
	return isSucess;
    }
     ...
     * @return a boolean tells if the operation succeeds
     */
    public <P extends Simple> boolean reduce(String contextName, String operationName, Table<P> table,
	    int reduceWorkerID) {
	boolean isSuccess = ReduceCollective.reduce(contextName, operationName, table, reduceWorkerID, dataMap,
		workers);
	dataMap.cleanOperationData(contextName, operationName);
	return isSuccess;
    }
     ...
     * @param table
     *            the table to hold the partitions
     * @return a boolean tells if the operations succeeds
     */
    public <P extends Simple> boolean allgather(String contextName, String operationName, Table<P> table) {
	boolean isSuccess = AllgatherCollective.allgather(contextName, operationName, table, dataMap, workers);
	dataMap.cleanOperationData(contextName, operationName);
	return isSuccess;
    }
	...
	boolean isSuccess = AllreduceCollective.allreduce(contextName, operationName, table, dataMap, workers);
	dataMap.cleanOperationData(contextName, operationName);
	return isSuccess;
    }
     ...
     *            the table to hold the partitions
     * @param partitioner
     *            a boolean tells if the operations succeeds
     * @return
     */
    public <P extends Simple, PT extends Partitioner> boolean regroup(String contextName, String operationName,
	    Table<P> table, PT partitioner) {
	boolean isSucess = RegroupCollective.regroupCombine(contextName, operationName, table, partitioner, dataMap,
		workers);
	dataMap.cleanOperationData(contextName, operationName);
	return isSucess;
    }
    ...
    /**
     * Pull partitions in the global table to the local tables. If any partition
     * ID conflicts with the partition ID in the local table, combines them.
     * 
     * @param contextName
     *            the name of the operation context
     * @param operationName
     ...
     *            the local tables
     * @param globalTable
     *            the global table, acts like a distributed dataset
     * @param useBcast
     *            if using broadcast in scattering the partitions
     * @return a boolean tells if the operation succeeds
     */
    public <P extends Simple> boolean pull(String contextName, String operationName, Table<P> localTable,
	    Table<P> globalTable, boolean useBcast) {
	boolean isSuccess = LocalGlobalSyncCollective.pull(contextName, operationName, localTable, globalTable,
     ...
     *            the local tables
     * @param globalTable
     *            the global table which acts like a distributed dataset, each
     *            partition in this table is unique
     * @param partitioner
     *            when a partiitoner is used, the local partitions are sent to
     *            the global table even without partiiton ID association
     * @return a boolean tells if the operation succeeds
     */
    public <P extends Simple, PT extends Partitioner> boolean push(String contextName, String operationName,
	    Table<P> localTable, Table<P> globalTable, PT partitioner) {
	boolean isSuccess = LocalGlobalSyncCollective.push(contextName, operationName, localTable, globalTable,
    ...
    /**
     * The rotation communication. Transfer the partitions from worker to
     * worker. After rotation, every worker sends its partitions to one of the
     * other worker, and receive partitions from one of the other workers
     * 
     * @param contextName
     *            the name of the operation context
     * @param operationName
     ...
     *            partition in this table is unique
     * @param rotateMap
     *            the map from worker to worker, defines how to rotate the data
     * @return true if succeeded, false if not
     */
    public <P extends Simple> boolean rotate(String contextName, String operationName, Table<P> globalTable,
	    Int2IntMap rotateMap) {
	boolean isSuccess = LocalGlobalSyncCollective.rotate(contextName, operationName, globalTable, rotateMap,
		dataMap, workers);
	dataMap.cleanOperationData(contextName, operationName);
	return isSuccess;
    }
     ...
     * 
     * @return an event object, null if the queue is empty
     */
    ...
    /**
     * Send an event to the local (local event), to a remote worker (message
     * event), or to the rest workers (collective event).
     * 
     * @param event
     *            an event
     * @return true if succeeded, false if not
	...
	if (event.getEventType() == EventType.LOCAL_EVENT && event.getBody() != null) {
	    eventQueue.addEvent(new Event(event.getEventType(), event.getContextName(), this.workerID, this.workerID,
		    event.getBody()));
	    return true;
	...
	LOG.info("Total Memory (bytes): " + " " + Runtime.getRuntime().totalMemory() + ", Free Memory (bytes): "
