 ...
 ******************************************************/
public class DataMSTBcastReceiver extends Receiver {

    @SuppressWarnings("unused")
    private static final Logger LOG = Logger.getLogger(DataMSTBcastReceiver.class);
     ...
     * @throws Exception
     */
    public DataMSTBcastReceiver(int selfID, ServerConn conn, EventQueue queue, DataMap map, Workers w, byte commandType)
	    throws Exception {
	super(conn, queue, map, commandType);
	this.selfID = selfID;
	workers = w;
	if (selfID == Constant.UNKNOWN_WORKER_ID) {
	    throw new Exception("Fail to initialize receiver.");
	}
    }
	...
	if (this.getCommandType() == Constant.MST_BCAST_DECODE) {
	    (new Decoder(data, selfID, EventType.COLLECTIVE_EVENT, this.getEventQueue(), this.getDataMap())).fork();
	} else {
	    // If the data is not for operation,
	    // put it to the queue with collective event
	    // type
	    DataUtil.addDataToQueueOrMap(selfID, this.getEventQueue(), EventType.COLLECTIVE_EVENT, this.getDataMap(),
		    data);
	}
    }
     ...
     * @throws Exception
     */
    private Data receiveData(final ServerConn conn) throws Exception {
	...
	if (left < right) {
	    ...
	    try {
		sendDataInMST(opArray.get(), opArray.size(), headArray.get(), headArray.size(), bodyArray.get(),
     ...
     * @throws IOException
     */
    private void sendDataInMST(byte[] opBytes, int opArrSize, byte[] headBytes, int headArrSize, byte[] bodyBytes,
	    int bodyArrSize, int left, int right) throws IOException {
	...
	while (left < right) {
	    ...
	    // + destLeft + " " + destRight);
	    Serializer serializer = new Serializer(new ByteArray(opBytes, 4, 8));
	    serializer.writeInt(destLeft);
	    serializer.writeInt(destRight);
	    // Send data to dest
	    WorkerInfo destWorker = workers.getWorkerInfo(destID);
	    if (destWorker != null) {
		Connection destConn = Connection.create(destWorker.getNode(), destWorker.getPort(), true);
		if (destConn != null) {
		    OutputStream out = destConn.getOutputStream();
		    // Send head and body array
		    if (out != null) {
			try {
			    out.write(getCommandType());
			    IOUtil.sendBytes(out, opBytes, 0, opArrSize);
			    if (headArrSize > 0) {
				IOUtil.sendBytes(out, headBytes, 0, headArrSize);
			    }
			    IOUtil.sendBytes(out, bodyBytes, 0, bodyArrSize);
			    destConn.release();
			} catch (Exception e) {
			    destConn.free();
			    throw e;
