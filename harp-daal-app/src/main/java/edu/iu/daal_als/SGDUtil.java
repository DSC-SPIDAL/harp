...
public class SGDUtil {
  ...
  /**
   * @brief read in COO format Sparse matrix data and grouped by row ids
   *
   * @param vFilePaths
   * @param numThreads
   * @param configuration
  ...
  public static
    ...
    loadVMapRow(LinkedList<String> vFilePaths,
      int numThreads, Configuration configuration) {
    VStore vStore =
      new VStore(vFilePaths, numThreads, false,
        configuration);
  ...
  public static
    Int2ObjectOpenHashMap<VRowCol>
    loadTMapRow(String testFilePath,
      int numThreads, Configuration configuration) {
    ...
    VStore vStore =
      new VStore(testFilePaths, numThreads, false,
        configuration);
    vStore.load(false, true);
    return vStore.getVWMap();
  ...
  /**
   * @brief read in COO format Sparse matrix in tran and group by row ids 
   *
   * @param vFilePaths
   * @param numThreads
   * @param configuration
  ...
  public static
    Int2ObjectOpenHashMap<VRowCol>
    loadVMapTran(LinkedList<String> vFilePaths,
      int numThreads, Configuration configuration) {
    VStore vStore =
      new VStore(vFilePaths, numThreads, true,
        configuration);
    vStore.load(false, true);
    return vStore.getVWMap();
  }
