...
public class ConnPool {
    ...
    private static final Logger LOG = Logger.getLogger(ConnPool.class);

    private static ConnPool instance = null;

    private Object2ObjectOpenHashMap<HostPort, Pool> connMap = new Object2ObjectOpenHashMap<>();
	...
	public boolean equals(Object object) {
	    ...
	    if (this.host.equals(hp.host) && this.port == hp.port) {
		return true;
	    } else {
		return false;
	    }
     ...
     */
    synchronized Connection getConn(String host, int port, boolean useCache) {
	if (useCache) {
		...
		Connection conn = pool.freeQueue.removeFirst();
		pool.inUseSet.add(conn);
		return conn;
	    } else {
		Connection conn = newConn(host, port, useCache);
		if (conn != null) {
		    pool.inUseSet.add(conn);
		}
		return conn;
     ...
     * @return the connection object
     */
    private Connection newConn(String host, int port, boolean useCache) {
	...
	do {
	    isFailed = false;
	    try {
		conn = new Connection(host, port, 0, useCache);
	    } catch (Exception e) {
		isFailed = true;
		count++;
		LOG.error("Error when connecting " + host + ":" + port + ", " + e.getMessage());
		try {
		    Thread.sleep(Constant.LONG_SLEEP);
		} catch (Exception e1) {
		}
	    }
	} while (isFailed && count < Constant.SMALL_RETRY_COUNT);
	if (isFailed) {
    ...
    synchronized boolean releaseConn(Connection conn) {
	if (conn == null) {
	    return false;
	}
	HostPort hostPort = new HostPort(conn.getNode(), conn.getPort());
	Pool pool = connMap.get(hostPort);
	if (pool != null) {
	...
	try {
	    conn.getOutputStream().write(Constant.CONNECTION_END);
	...
	ObjectIterator<Object2ObjectMap.Entry<HostPort, Pool>> iterator = connMap.object2ObjectEntrySet()
		.fastIterator();
	while (iterator.hasNext()) {
	    Object2ObjectMap.Entry<HostPort, Pool> entry = iterator.next();
	    HostPort hostPort = entry.getKey();
	    LOG.info("connection=" + hostPort.host + ":" + hostPort.port + ", use=" + entry.getValue().inUseSet.size()
		    + ", released=" + entry.getValue().freeQueue.size());
	}
    }
}
