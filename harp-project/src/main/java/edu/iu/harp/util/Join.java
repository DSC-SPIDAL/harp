...
/*******************************************************
 * This class records the destinations of each partition. Each partition could
 * have multi-destinations.
 ******************************************************/
...
public class Join extends Writable {
     ...
     *            the parToWorkerMap
     */
    public void setParToWorkerMap(Int2ObjectOpenHashMap<IntArrayList> partitionToWorkerMap) {
     ...
     */
    public Int2ObjectOpenHashMap<IntArrayList> getParToWorkerMap() {
     ...
     */
    public void setWorkerParCountMap(Int2IntOpenHashMap workerPartitionCountMap) {
	this.workerParCountMap = workerPartitionCountMap;
    }
     ...
     */
    public Int2IntOpenHashMap getWorkerParCountMap() {
	...
	if (parToWorkerMap != null) {
	    ObjectIterator<Int2ObjectMap.Entry<IntArrayList>> iterator = parToWorkerMap.int2ObjectEntrySet()
		    .fastIterator();
	    while (iterator.hasNext()) {
		Int2ObjectMap.Entry<IntArrayList> entry = iterator.next();
	...
	if (workerParCountMap != null) {
	    ...
	    for (Int2IntMap.Entry entry : this.workerParCountMap.int2IntEntrySet()) {
		out.writeInt(entry.getIntKey());
	...
	if (mapSize > 0) {
	    if (parToWorkerMap == null) {
		parToWorkerMap = new Int2ObjectOpenHashMap<IntArrayList>(mapSize);
	    }
	}
	...
	if (mapSize > 0) {
	    if (workerParCountMap == null) {
		workerParCountMap = new Int2IntOpenHashMap(mapSize);
	    }
	}
	for (int i = 0; i < mapSize; i++) {
	    workerParCountMap.put(in.readInt(), in.readInt());
	}
    }
