...
DAAL_EXPORT Status Result::allocate(const daal::algorithms::Input *input, const daal::algorithms::Parameter *par, const int method)
{
    ...
    {
        Argument::set(value, SerializationIfacePtr(new HomogenNumericTable<algorithmFPType>(inputTable->getNumberOfColumns(), inputTable->getNumberOfRows(), NumericTable::doAllocate)));
    }
    return s;
}
