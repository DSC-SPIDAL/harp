...
public final class ByteArray extends Array<byte[]> {
     ...
     * four bytes for storing the size, size bytes for storing the data.
     */
    @Override
     ...
     * @return
     */
    public static ByteArray create(int len, boolean approximate) {
	if (len > 0) {
	    byte[] bytes = ResourcePool.get().getBytesPool().getArray(len, approximate);
	    if (bytes != null) {
    ...
    @Override
    public void release() {
	ResourcePool.get().getBytesPool().releaseArray(array);
	this.reset();
    }
    ...
    @Override
    public void free() {
	ResourcePool.get().getBytesPool().freeArray(array);
	this.reset();
