...
{                                                                                                                           \
    dnnError_t err;                                                                                                         \
                                                                                                                            \
    if (_isPlainLayout && !_isDnnLayout && _dnnLayout != NULL && _plainPtr != NULL && _dnnPtr != NULL)                      \
    {                                                                                                                       \
        err = layoutConvertCpu<T>(&_plainPtr, (dnnLayout_t)_plainLayout, true, &_dnnPtr, (dnnLayout_t)_dnnLayout, true);    \
        _isDnnLayout = true;                                                                                                \
    }                                                                                                                       \
    return services::Status();                                                                                              \
}                                                                                                                           \
...
{                                                                                                                           \
    dnnError_t err;                                                                                                         \
                                                                                                                            \
    if (_isDnnLayout && !_isPlainLayout && _dnnLayout != NULL && _plainPtr != NULL && _dnnPtr != NULL)                      \
    {                                                                                                                       \
        err = layoutConvertCpu<T>(&_dnnPtr, (dnnLayout_t)_dnnLayout, true, &_plainPtr, (dnnLayout_t)_plainLayout, true);    \
        _isPlainLayout = true;                                                                                              \
    }                                                                                                                       \
    return services::Status();                                                                                              \
}                                                                                                                           \
...
{                                                                                                                \
    if (_dnnLayout != NULL)                                                                                      \
    {                                                                                                            \
        layoutDeleteCpu<T>((dnnLayout_t)_dnnLayout);                                                             \
    }                                                                                                            \
                                                                                                                 \
    _dnnLayout = NULL;                                                                                           \
    _isDnnLayout = false;                                                                                        \
...
{                                                                                                                \
    ...
    {                                                                                                            \
        layoutDeleteCpu<T>((dnnLayout_t)_plainLayout);                                                           \
    }                                                                                                            \
                                                                                                                 \
    _plainLayout = NULL;                                                                                         \
    _isPlainLayout = false;                                                                                      \
...
{                                                                                                                \
    ...
    {                                                                                                            \
        ...
        if (_isDnnLayout)                                                                                        \
        {                                                                                                        \
            bool sameLayout = layoutCompareCpu<T>((dnnLayout_t)_dnnLayout, (dnnLayout_t)newDnnLayout);           \
            if (!sameLayout) {                                                                                   \
                err = allocateBufferCpu<T>((void**)&newPtr, (dnnLayout_t)newDnnLayout); ON_ERR(err);             \
                err = layoutConvertCpu<T>(&_dnnPtr, (dnnLayout_t)_dnnLayout, true, &newPtr, (dnnLayout_t)newDnnLayout, true);  \
                ON_ERR(err);                                                                                     \
            }                                                                                                    \
            else                                                                                                 \
            {                                                                                                    \
        ...
        {                                                                                                        \
            err = allocateBufferCpu<T>((void**)&newPtr, (dnnLayout_t)newDnnLayout); ON_ERR(err);                 \
            err = layoutConvertCpu<T>(&_plainPtr, (dnnLayout_t)_plainLayout, true, &newPtr, (dnnLayout_t)newDnnLayout, true);  \
            ON_ERR(err);                                                                                         \
        }                                                                                                        \
                                                                                                                 \
...
{   services::Status s;                                                                                          \
    ...
    {                                                                                                            \
        ...
        return s;                                                                                                \
                                                                                                                 ...
                                                                                                                 \
    err = allocateBufferCpu<T>((void**)&_plainPtr, (dnnLayout_t)_plainLayout);                                   \
    ON_ERR(err);                                                                                                 \
                                                                                                                 \
    if( _plainPtr == 0 )                                                                                         \
    {                                                                                                            \
    ...
    if (_dnnLayout != NULL)                                                                                      \
    {                                                                                                            \
        err = allocateBufferCpu<T>((void**)&_dnnPtr, (dnnLayout_t)_dnnLayout);                                   \
        ON_ERR(err);                                                                                             \
                                                                                                                 \
        if( _dnnPtr == 0 )                                                                                       \
        {                                                                                                        \
...
{                                                                                                                \
    ...
    delete [] newStrides;                                                                                        \
                                                                                                                 \
    if (_plainLayout != NULL && getDataMemoryStatus () != notAllocated)                                          \
    {                                                                                                            \
        syncDnnToPlain();                                                                                        \
        T *newPtr = NULL;                                                                                        \
        err = allocateBufferCpu<T>((void**)&newPtr, (dnnLayout_t)_plainLayout); ON_ERR(err);                     \
        err = layoutConvertCpu<T>(&_plainPtr, (dnnLayout_t)_plainLayout, true, &newPtr, (dnnLayout_t)newDnnLayout, true);  \
        ON_ERR(err);                                                                                             \
    ...
    _plainLayout = (void*)newDnnLayout;                                                                          \
    _isPlainLayout = true;                                                                                       \
    return services::Status();                                                                                   \
}                                                                                                                \
                                                                                                                 \
