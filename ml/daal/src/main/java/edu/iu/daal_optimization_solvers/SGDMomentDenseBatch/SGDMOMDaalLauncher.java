...
public class SGDMOMDaalLauncher extends Configured
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 11) {
      System.err
        .println("Usage: edu.iu.daal_svd.SGDMOMDaalLauncher "
          + "<file dim > "
	  + "<nfeature>"
	  + "<accuracyThreshold>"
	  + "<nIterations>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runSGDMOM(nFileDim, nFeatures, accuracyThreshold, nIterations, batchSize, learningRate, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total SGDMOM Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runSGDMOM(
    ...
    Job SGDMOMJob =
      configureSGDMOMJob(nFileDim, nFeatures, accuracyThreshold, nIterations, batchSize, learningRate,
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      SGDMOMJob.waitForCompletion(true);
    System.out
      .println("end job "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("SGDMOM Job fails.");
    }
  }

  private Job configureSGDMOMJob(
    ...
    Job job =
      Job
        .getInstance(configuration, "SGDMOM_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(SGDMOMDaalLauncher.class);
    job
      .setMapperClass(SGDMOMDaalCollectiveMapper.class);
