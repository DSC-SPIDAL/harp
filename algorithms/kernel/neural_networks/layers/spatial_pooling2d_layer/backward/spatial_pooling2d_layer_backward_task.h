...
public:
    ...
    Status init()
    {
        ...
        poolingInputDimensions.reset(new HomogenNumericTable<algorithmFPType>(nDims, 1 , NumericTable::doAllocate));
    ...
    Status prepareCommonPoolingTensors(size_t level, size_t offset)
    {
        ...
        poolingInputGradientTensor.reset(new HomogenTensor<algorithmFPType>(inputGradientSliceDims, const_cast<algorithmFPType *>(inputGradientData)));

        poolingGradientTensor.reset(new HomogenTensor<algorithmFPType>(gradientSliceDims, poolingGradientArray.get()));
        return Status();
    }
    ...
    Status preparePoolingParameter(size_t level)
    {
        ...
        poolingParameter.paddings.size[0] = (poolingParameter.kernelSizes.size[0] * pow2 - gradientSliceDims[2] + 1) / 2;
        poolingParameter.paddings.size[1] = (poolingParameter.kernelSizes.size[1] * pow2 - gradientSliceDims[3] + 1) / 2;
        return Status();
    }
...
public:
    ...
    Status preparePoolingTensors(size_t level, size_t offset) DAAL_C11_OVERRIDE
    {
        ...
        poolingSelectedPosTensor.reset(new HomogenTensor<algorithmFPType>(this->inputGradientSliceDims,  const_cast<algorithmFPType *>(selectedPosData)));
        return s;
    }
