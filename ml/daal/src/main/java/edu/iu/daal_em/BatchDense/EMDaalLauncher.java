...
public class EMDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 7) {
      System.err
        .println("Usage: edu.iu.daal_svd.EMDaalLauncher "
          + "<file dim > "
	  + "<nComponents>"
	  + "<number of map tasks>"
	  + "<num threads> "
    ...
    launch(nFileDim, nComponents, numMapTasks, numThreads, mem, trainDataDir, workDir);

    return 0;
  }

  private void launch(int nFileDim, int nComponents, 
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runEMMultiDense(nFileDim, nComponents, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total EM Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runEMMultiDense(
    int nFileDim, int nComponents, 
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job EMJob =
      configureEMJob(nFileDim, nComponents, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("EM Job fails.");
    }
  }

  private Job configureEMJob(
    int nFileDim, int nComponents, 
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "EM_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(EMDaalLauncher.class);
    job
      .setMapperClass(EMDaalCollectiveMapper.class);
    ...
    jobConfig.setInt(Constants.NUM_THREADS,
      numThreads);
    jobConfig.setInt(Constants.NUM_COMPONENTS,
      nComponents);
