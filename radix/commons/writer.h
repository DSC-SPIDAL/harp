...
class WriterBase {
 public:
  explicit WriterBase(RCSRGraph<NodeID_, DestID_> &g) : g_(g) {}
  ...
  // writing as column major
  void WriteMTX(std::fstream &out) {
    out << g_.num_nodes() << " " << g_.num_nodes() << " " << g_.num_edges_directed() << std::endl;
    for (NodeID_ u=0; u < g_.num_nodes(); u++) {
      for (DestID_ v : g_.out_neigh(u))
        out << v+1 << " " << u+1 << std::endl;
    }
  }
