        ...
        Tensor *selectedPosTensor, const spatial_pooling2d::Parameter &spatialParameter)
{
    ...
    for(size_t slice = 0; slice < dims[0]; slice++)
    {
        ...
        const HomogenTensor<algorithmFPType> dataSliceTensor(dataSliceDims, const_cast<algorithmFPType *>(dataSlice));
        ...
        for(size_t level = 0, pow2 = 1; level < L ; level++, pow2 *= 2)
        {
            ...
            HomogenTensor<algorithmFPType> poolingValueTensor(valueDims, value);

            TensorPtr poolingSelectedPosTensor;
            if(selectedPosTensor)
            {
                int *selectedPos = &selectedPosSlice[accumulatedFlattenOffset];
                poolingSelectedPosTensor.reset(new HomogenTensor<int>(valueDims, selectedPos));
            }
    ...
    Tensor *poolingSelectedPosTensor)
{
    ...
    return poolKernel.compute(dataTensor, poolingValueTensor, poolingSelectedPosTensor, forwardStochasticParameter);
}
