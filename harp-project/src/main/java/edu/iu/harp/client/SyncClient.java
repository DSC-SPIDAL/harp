...
public class SyncClient implements Runnable {
	...
	/** Generated serial ID */
	private static final long serialVersionUID = 2147825101521531758L;
	...
	if (event.getEventType() == EventType.MESSAGE_EVENT && event.getBody() != null) {
	    if (event.getTargetID() != selfID) {
		SyncQueue queue = getSyncQueue(event.getTargetID(), event.getContextName());
     ...
     * @return
     */
    public boolean submitCollectiveEvent(Event event) {
	// Add event to the queue
	// Collective event to a special queue
	// Add queue to the consumer blocking queue
	if (event.getEventType() == EventType.COLLECTIVE_EVENT && event.getBody() != null) {
	    SyncQueue queue = getSyncQueue(ALL_WORKERS, event.getContextName());
	    queue.add(event.getBody());
	    return true;
	} else {
	    return false;
     ...
     */
    private SyncQueue getSyncQueue(int destID, String contextName) {
	ConcurrentMap<String, SyncQueue> queueMap = eventQueueMap.get(destID);
	if (queueMap == null) {
	    queueMap = new ConcurrentHashMap<>();
	    ConcurrentMap<String, SyncQueue> oldQueueMap = eventQueueMap.putIfAbsent(destID, queueMap);
	    if (oldQueueMap != null) {
	...
	if (queue == null) {
	    queue = new SyncQueue(contextName, selfID, destID, workers, consumerQueue);
	    SyncQueue oldQueue = queueMap.putIfAbsent(contextName, queue);
	    if (oldQueue != null) {
		queue = oldQueue;
	    }
	}
    ...
    /**
     * Main process by this client Go through each queue, drain the queue and
     * send/broadcast
     */
    @Override
    public void run() {
	while (true) {
		...
		LOG.error("Error when taking from the queue", e);
		continue;
	    }
     ...
     */
    public void stop() {
	SyncQueue queue = getSyncQueue(NO_WORKERS, "");
	queue.add(null);
