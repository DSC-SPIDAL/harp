...
public class CLYDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 6) {
      System.err
        .println("Usage: edu.iu.daal_svd.CLYDaalLauncher "
          + "<file dim > "
	  + "<number of map tasks>"
	  + "<num threads> "
          + "<mem>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    System.out
      .println("Total CLY Multi Dense Execution Time: "
        + (endTime - startTime));
  }
  ...
  private void runCLY(
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("CLY Job fails.");
    }
  }

  private Job configureCLYJob(
    int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "CLY_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(CLYDaalLauncher.class);
    job
      .setMapperClass(CLYDaalCollectiveMapper.class);
