...
public class GraphCollective {
    ...
    private static final Logger LOG = Logger.getLogger(GraphCollective.class);
	...
	LOG.info("args[] " + driverHost + " " + driverPort + " " + workerID + " " + jobID);
	// ------------------------------------------------
	// Worker initialize
	EventQueue dataQueue = new EventQueue();
	DataMap dataMap = new DataMap();
	Workers workers = new Workers(workerID);
	Server server = new Server(workers.getSelfInfo().getNode(), workers.getSelfInfo().getPort(), dataQueue, dataMap,
		workers);
	server.start();
	...
	int totalVtxCount = vtxCountPerWorker * numWorkers;
	...
	try {
		...
		int target = random.nextInt(totalVtxCount);
		int source = 0;
		do {
		    source = random.nextInt(totalVtxCount);
		} while (source == target);
		val.addEdge(source, 0, target);
		ValStatus status = inEdgeTable.addKeyVal(target, val);
	...
	try {
	    ...
	    isSuccess = RegroupCollective.regroupCombine(contextName, "regroup-edges", inEdgeTable,
		    new Partitioner(numWorkers), dataMap, workers);
	...
	try {
	    for (Partition<EdgePartition> partition : inEdgeTable.getPartitions()) {
		ObjectIterator<Int2ObjectMap.Entry<EdgeVal>> iterator = partition.get().getKVMap().int2ObjectEntrySet()
			.fastIterator();
		while (iterator.hasNext()) {
		    EdgeVal edgeVal = iterator.next().getValue();
		    for (int i = 0; i < edgeVal.getNumEdges(); i++) {
			vtxTable.addKeyVal(edgeVal.getDest()[i], new IntVal(1));
		    }
		}
	    }
	} catch (Exception e) {
	    LOG.error("Fail to create vertex table from the edge table.", e);
	}
	...
	for (Partition<EdgePartition> partition : inEdgeTable.getPartitions()) {
		...
		for (int i = 0; i < edgeVal.getNumEdges(); i++) {
		    msgVal.setInt(1);
		    ValStatus status = msgTable.addKeyVal(edgeVal.getSrc()[i], msgVal);
		    if (status == ValStatus.ADDED) {
			msgVal = new IntVal();
		    }
		}
	...
	try {
	    join(contextName, "send-msg-to-vtx", msgTable, null, vtxTable, dataMap, workers);
	} catch (Exception e) {
     ...
     */
    private static void printEdgeTable(EdgeTable edgeTable) {
	for (Partition<EdgePartition> partition : edgeTable.getPartitions()) {
	    ObjectIterator<Int2ObjectMap.Entry<EdgeVal>> iterator = partition.get().getKVMap().int2ObjectEntrySet()
		    .fastIterator();
	    while (iterator.hasNext()) {
		EdgeVal edgeVal = iterator.next().getValue();
		for (int i = 0; i < edgeVal.getNumEdges(); i++) {
		    LOG.info("Partiiton ID: " + partition.id() + ", Edge: " + edgeVal.getSrc()[i] + "->"
			    + edgeVal.getDest()[i]);
		}
	    }
	}
     ...
     */
    private static void printMsgTable(MessageTable msgTable) {
	for (Partition<MessagePartition> partition : msgTable.getPartitions()) {
	    ObjectIterator<Int2ObjectMap.Entry<IntVal>> iterator = partition.get().getKVMap().int2ObjectEntrySet()
		    .fastIterator();
	    while (iterator.hasNext()) {
		Int2ObjectMap.Entry<IntVal> entry = iterator.next();
		int key = entry.getIntKey();
		IntVal msgVal = entry.getValue();
		LOG.info("Partiiton ID: " + partition.id() + ", Msg: " + key + "->" + msgVal.getInt());
	    }
	}
    }
     ...
     */
    private static void printVtxTable(VertexTable vtxTable) {
	for (Partition<VertexPartition> partition : vtxTable.getPartitions()) {
	    ObjectIterator<Int2ObjectMap.Entry<IntVal>> iterator = partition.get().getKVMap().int2ObjectEntrySet()
		    .fastIterator();
	    while (iterator.hasNext()) {
		Int2ObjectMap.Entry<IntVal> entry = iterator.next();
		...
		LOG.info("Partiiton ID: " + partition.id() + ", Val: " + key + "->" + vtxVal.getInt());
	    }
	}
    }
     ...
     */
    public static <P1 extends Simple, P2 extends Simple> boolean join(String contextName, String operationName,
	    Table<P1> dynamicTable, Partitioner partitioner, Table<P2> staticTable, DataMap dataMap, Workers workers) {
	...
	LinkedList<Transferable> recvStaticPSets = new LinkedList<>();
	boolean isSuccess = PartitionUtil.gatherPartitionSet(contextName, operationName + ".gatherstatic", staticTable,
		recvStaticPSets, dataMap, workers);
	if (!isSuccess) {
	    return false;
	}
	// ---------------------------------------------------
	...
	LinkedList<Transferable> recvDynamicPSets = new LinkedList<>();
	isSuccess = PartitionUtil.gatherPartitionSet(contextName, operationName + ".gatherdynamic", dynamicTable,
		recvDynamicPSets, dataMap, workers);
	// LOG.info("Dynamic table info is gathered.");
	if (!isSuccess) {
	    return false;
	}
	// ---------------------------------------------------
	...
	LinkedList<Transferable> recvJoin = new LinkedList<>();
	if (workers.isMaster()) {
	    Join join = createJoin(recvStaticPSets, recvDynamicPSets, partitioner);
	...
	isSuccess = Communication.mstBcastAndRecv(contextName, masterID, operationName + ".bcast", recvJoin, workers,
		dataMap);
	dataMap.cleanOperationData(contextName, operationName + ".bcast");
	...
	LinkedList<Partition<P1>> bcastPartitions = new LinkedList<>();
	@SuppressWarnings("unchecked")
	LinkedList<Partition<P1>>[] sendPartitionMap = new LinkedList[numWorkers];
	int numSendWorkers = 0;
	IntArrayList rmPartitionIDs = new IntArrayList();
	int localCount = 0;
	for (Partition<P1> partition : dynamicTable.getPartitions()) {
	    int partitionID = partition.id();
	    IntArrayList workerIDs = join.getParToWorkerMap().get(partitionID);
	    if (workerIDs != null) {
	...
	if (!bcastPartitions.isEmpty()) {
	    LocalGlobalSyncCollective.broadcast(contextName, operationName, bcastPartitions, workers);
	}
	if (numSendWorkers > 0) {
	    LocalGlobalSyncCollective.dispatch(contextName, operationName, sendPartitionMap, workers);
	}
	// --------------------------------------------------
	// Receive all the partitions
	int numRecvPartitions = join.getWorkerParCountMap().get(selfID) - localCount;
	// LOG.info("Total receive: "
	// + numRecvPartitions);
	join.release();
	join = null;
	return PartitionUtil.receivePartitions(contextName, operationName, dynamicTable, numRecvPartitions,
		rmPartitionIDs, dataMap);
    }

    /**
     * Generate partition and worker mapping for join
     * 
     ...
     */
    private static Join createJoin(LinkedList<Transferable> recvStaticPSets, LinkedList<Transferable> recvDynamicPSets,
	    Partitioner partitioner) {
	// partition <-> workers mapping
	// based on static partition info
	Int2ObjectOpenHashMap<IntArrayList> parToWorkerMap = new Int2ObjectOpenHashMap<>();
	for (Transferable trans : recvStaticPSets) {
	    ...
	    for (int pID : pList) {
		IntArrayList destWorkerIDs = parToWorkerMap.get(pID);
	...
	Int2IntOpenHashMap workerParCountMap = new Int2IntOpenHashMap();
	for (Transferable trans : recvDynamicPSets) {
		...
		if (destWorkerIDs != null) {
		    ...
		    int destWorkerID = partitioner.getWorkerID(pID);
		    if (destWorkerID != Constant.UNKNOWN_WORKER_ID) {
			destWorkerIDs = new IntArrayList();
			parToWorkerMap.put(pID, destWorkerIDs);
			destWorkerIDs.add(destWorkerID);
			workerParCountMap.addTo(destWorkerID, 1);
		    }
		}
	    }
	}
