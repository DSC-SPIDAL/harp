...
{
    bernoulliParameter.engine = parameter.engine;
    bernoulliParameter.p      = parameter.retainRatio;
    return Status();
}
    ...
    const algorithmFPType inverseRetainRatio)
{
    ...
    DAAL_CHECK(bernoulliKernel.computeInt(rngBuffer, nDataElements, bernoulliParameter), ErrorIncorrectErrorcodeFromGenerator);
