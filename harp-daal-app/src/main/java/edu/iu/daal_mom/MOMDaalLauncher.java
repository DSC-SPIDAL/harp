...
ExecutionException, ClassNotFoundException {
  ...
  runMOM(inputDir, testDirPath, testGroundTruthDirPath, mem, batchSize, numMapTasks,
      numThreadsPerWorker, modelDir,
      outputDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total MOM Execution Time: "
        + (endTime - startTime));
}

private void runMOM(Path inputDir, String testDirPath, String testGroundTruthDirPath, int mem, int batchSize,  
    ...
    Job momJob =
      configureMOMJob(inputDir, testDirPath, testGroundTruthDirPath, mem, batchSize,
        numMapTasks, numThreadsPerWorker,
        modelDir, outputDir, configuration);

    boolean jobSuccess =
      momJob.waitForCompletion(true);
  ...
  private Job configureMOMJob(Path inputDir,
