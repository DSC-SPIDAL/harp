...
services::Status ImplicitALSInitKernelBase<algorithmFPType, cpu>::randFactors(
            size_t nItems, size_t nFactors, algorithmFPType *itemsFactors, int *buffer, engines::EnginePtr engine)
{
    const int randMax = 1000;
    const algorithmFPType invRandMax = 1.0 / algorithmFPType(randMax);

    distributions::uniform::Parameter<int> uniformParameter(0, randMax);
    ...
    {
        DAAL_CHECK(uniformKernel.compute(uniformParameter, nFactors - 1, buffer), ErrorIncorrectErrorcodeFromGenerator)
