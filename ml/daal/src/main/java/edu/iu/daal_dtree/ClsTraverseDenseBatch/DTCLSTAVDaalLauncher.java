...
public class DTCLSTAVDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 9) {
      System.err
        .println("Usage: edu.iu.daal_svd.DTCLSTAVDaalLauncher "
          + "<feature dim> "
          + "<file dim > "
	  + "<nClass>"
	  + "<number of map tasks>"
    ...
    launch(nFeature, nFileDim, nClass, numMapTasks, numThreads, mem, trainDataDir, pruneDataDir, workDir);
    return 0;
  }

  private void launch(int nFeature, int nFileDim, int nClass, 
		      int numMapTasks, int numThreads, int mem, 
		      String trainDataDir, String pruneDataDir, String workDir) throws IOException,
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runDTCLSTAVMultiDense(nFeature, nFileDim, nClass, 
		    numMapTasks, numThreads, mem, trainDataDir, pruneDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total DTCLSTAV Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runDTCLSTAVMultiDense(
    int nFeature, int nFileDim, int nClass, 
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, String pruneDataDir, 
    Path dataDir,
    ...
    Job DTCLSTAVJob =
      configureDTCLSTAVJob(nFeature, nFileDim, nClass, 
		    numMapTasks, numThreads, mem, trainDataDir, pruneDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      DTCLSTAVJob.waitForCompletion(true);
    System.out
      .println("end Jod "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("DTCLSTAV Job fails.");
    }
  }

  private Job configureDTCLSTAVJob(
    int nFeature, int nFileDim, int nClass, 
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, String pruneDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "DTCLSTAV_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(DTCLSTAVDaalLauncher.class);
    job
      .setMapperClass(DTCLSTAVDaalCollectiveMapper.class);
