 ...
 public:
  ...
  void reserve(size_t num_elements) {
    if (num_elements > capacity()) {
      ...
      T_ *new_range = (T_*) aligned_alloc(64, num_elements*sizeof(T_));
      if (start_ != nullptr) {
  ...
  T_* append_atomic(size_t num_elements) {
    if (num_elements + size() > capacity()) {
      std::cout << "grew too much" << std::endl;
      std::exit(-1);
    }
    return fetch_and_add(end_size_, num_elements*sizeof(T_));
  }
