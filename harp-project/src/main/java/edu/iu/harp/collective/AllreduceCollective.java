...
public class AllreduceCollective {
    ...
    private static final Logger LOG = Logger.getLogger(AllreduceCollective.class);
	...
	for (int i = 0; i < numPartitions; i++) {
	    ...
	    DoubleArray doubleArray = new DoubleArray(doubles, 0, doublesSize);
     ...
     */
    public static <P extends Simple> boolean allreduce(final String contextName, final String operationName,
	    final Table<P> table, final DataMap dataMap, final Workers workers) {
	...
	while (left < right) {
	    ...
	    // + destID);
	    List<Transferable> ownedPartitions = new LinkedList<>(table.getPartitions());
	    ...
	    if (!isDestAdjusted) {
		Data recvData = cachedDataMap.remove(destID);
		// Wait data
		if (recvData == null) {
		    while (true) {
			...
			if (recvData == null) {
			    ...
			    if (recvData.getWorkerID() != destID) {
		...
		// + middle);
		Data recvData = cachedDataMap.remove(middle);
		// Wait data
		if (recvData == null) {
		    while (true) {
			recvData = IOUtil.waitAndGet(dataMap, contextName, operationName);
			if (recvData == null) {
			    ...
			    recvData.releaseHeadArray();
			    recvData.releaseBodyArray();
			    if (recvData.getWorkerID() != middle) {
				cachedDataMap.put(recvData.getWorkerID(), recvData);
			    } else {
				break;
			    }
