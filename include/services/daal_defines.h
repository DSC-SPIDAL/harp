...
#define DAAL_RETURN_STATUS() return services::Status(*this->_errors);
#define DAAL_KERNEL_CHECK(cond) if(!(cond)) return services::Status(*this->_errors);
#define DAAL_KERNEL_CHECK_EX(cond, error) if(!(cond)) { this->_errors->add(error); return services::Status(*this->_errors); }
