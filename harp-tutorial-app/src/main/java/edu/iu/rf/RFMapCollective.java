...
public class RFMapCollective extends Configured
  implements Tool {

  public static void main(String[] args)
    throws Exception {
    int res = ToolRunner.run(new Configuration(),
      new RFMapCollective(), args);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    Job job = configureRFJob(numTrees,
      numMapTasks, numThreads, trainPath,
      testPath, outputPath, configuration);
    boolean jobSuccess =
      job.waitForCompletion(true);
    if (!jobSuccess) {
      System.out
        .println("Random Forests job fails.");
    } else {
      System.out
        .println("Random Forests job suceeds!");
    }
    return 0;
  }

  private Job configureRFJob(int numTrees,
    ...
    Job job =
      Job.getInstance(configuration, "RF_job");
    JobConf jobConf =
    ...
    jobConfiguration.setInt("numMapTasks",
      numMapTasks);
    jobConfiguration.setInt("numThreads",
      numThreads);
    jobConfiguration.set("trainPath", trainPath);
    jobConfiguration.set("testPath", testPath);
    jobConfiguration.set("outputPath",
