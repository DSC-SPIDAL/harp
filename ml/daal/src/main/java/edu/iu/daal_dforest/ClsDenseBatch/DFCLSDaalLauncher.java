...
public class DFCLSDaalLauncher extends Configured
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 11) {
      System.err
        .println("Usage: edu.iu.daal_svd.DFCLSDaalLauncher "
          + "<feature dim> "
          + "<file dim > "
	  + "<nClass>"
	  + "<ntrees>"
    ...
    launch(nFeature, nFileDim, nClass, nTrees, minObsLeafNodes, numMapTasks, numThreads, mem, trainDataDir, testDataDir, workDir);
    return 0;
  }

  private void launch(int nFeature, int nFileDim, int nClass, int nTrees, int minObsLeafNodes, 
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runDFCLSMultiDense(nFeature, nFileDim, nClass, nTrees, minObsLeafNodes, 
		    numMapTasks, numThreads, mem, trainDataDir, testDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total DFCLS Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runDFCLSMultiDense(
    int nFeature, int nFileDim, int nClass, int nTrees, int minObsLeafNodes, 
    int numMapTasks, int numThreads, int mem, 
    ...
    Job DFCLSJob =
      configureDFCLSJob(nFeature, nFileDim, nClass, nTrees, minObsLeafNodes, 
		    numMapTasks, numThreads, mem, trainDataDir, testDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      DFCLSJob.waitForCompletion(true);
    System.out
      .println("end Jod "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("DFCLS Job fails.");
    }
  }

  private Job configureDFCLSJob(
    int nFeature, int nFileDim, int nClass, int nTrees, int minObsLeafNodes, 
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, String testDataDir,
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "DFCLS_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(DFCLSDaalLauncher.class);
    job
      .setMapperClass(DFCLSDaalCollectiveMapper.class);
