...
public class DTREGTAVDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 8) {
      System.err
        .println("Usage: edu.iu.daal_svd.DTREGTAVDaalLauncher "
          + "<feature dim> "
          + "<file dim > "
	  + "<number of map tasks>"
	  + "<num threads> "
    ...
    launch(nFeature, nFileDim, numMapTasks, numThreads, mem, trainDataDir, pruneDataDir, workDir);
    return 0;
  }
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runDTREGTAV(nFeature, nFileDim,
		    numMapTasks, numThreads, mem, trainDataDir, pruneDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total DTREGTAV Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runDTREGTAV(
    ...
    Job DTREGTAVJob =
      configureDTREGTAVJob(nFeature, nFileDim,
		    numMapTasks, numThreads, mem, trainDataDir, pruneDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      DTREGTAVJob.waitForCompletion(true);
    System.out
      .println("end Jod "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("DTREGTAV Job fails.");
    }
  }

  private Job configureDTREGTAVJob(
    int nFeature, int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, String pruneDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "DTREGTAV_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(DTREGTAVDaalLauncher.class);
    job
      .setMapperClass(DTREGTAVDaalCollectiveMapper.class);
