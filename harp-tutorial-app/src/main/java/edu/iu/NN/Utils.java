...
public class Utils {
  ...
  /**
   * Gets a matrix from a ascii text file. The
   * format of the text file should have each file
   * containing only one matrix. The format of the
   * text file should have each row separated by a
   * newline character The format of the text file
   * should have each column separated by a space
   * character (' ').
   */

  public static DoubleMatrix
    getMatrixFromTextFile(String filename,
      Configuration conf)
      throws NumberFormatException, IOException {

    FileSystem fs = FileSystem.get(conf);
    Path dPath =
      new Path("hdfs://j-045:9010/" + filename);
  ...
  static File readData(int numMapTasks,
    FileSystem fs, String localDirStr,
    Path dataDir, Configuration conf)
    throws IOException, InterruptedException,
    ExecutionException {

    DoubleMatrix X = getMatrixFromTextFile(
      "/digitInput.txt", conf);
    DoubleMatrix Y = getMatrixFromTextFile(
      "/digitOutput.txt", conf);
    ...
    System.out.println(
      "Writing " + miniBatchSize + " vectors to "
    ...
    for (int k = 0; k < numMapTasks; k++) {
      try {
        ...
        File fileX = new File(
          localDirStr + File.separator + "batch_"
            + Integer.toString(kk++) + filename);
        File fileY = new File(
          localDirStr + File.separator + "batch_"
            + Integer.toString(kk++) + filename);
        FileWriter fwX =
          new FileWriter(fileX.getAbsoluteFile());
        FileWriter fwY =
          new FileWriter(fileY.getAbsoluteFile());
        BufferedWriter bwX =
          new BufferedWriter(fwX);
        BufferedWriter bwY =
          new BufferedWriter(fwY);
