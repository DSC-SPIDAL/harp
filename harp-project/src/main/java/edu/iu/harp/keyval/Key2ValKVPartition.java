...
/*******************************************************
 * Key2ValKVPartition manages key-value pairs in which the key and the value are
 * objects
 ******************************************************/
public abstract class Key2ValKVPartition<K extends Key, V extends Value> extends KVPartition {

    private static final Logger LOG = Logger.getLogger(Key2ValKVPartition.class);
     ...
     *            the class of Value
     */
    public void initialize(Class<K> kClass, Class<V> vClass) {
	if (this.kvMap == null) {
	    this.kvMap = new Object2ObjectOpenHashMap<>();
    ...
    /**
     * Put the new key-value pair to the partition. If the key already exists in
     * the partition, combine the original value with the new value; else, add
     * the new key-value pair to the partition
     * 
     * @param key
     *            the new key
     * @param val
     ...
     */
    public ValStatus putKeyVal(K key, V val, ValCombiner<V> combiner) {
    ...
    /**
     * Removes this key and the associated value from this function if it is
     * present.
     * 
     * @param key
     *            the key
     * @return the associated value, or null if no value was present for the
     *         given key.
     */
     ...
     */
    public Object2ObjectOpenHashMap<K, V> getKVMap() {
	return kvMap;
    }
    ...
    /**
     * Clear the partition. All Key objects and Value objects will be cached in
     * freeKeys and freeVales respectively for later reuse
     */
    @Override
    public void clear() {
	if (!this.kvMap.isEmpty()) {
	    for (Object2ObjectMap.Entry<K, V> entry : this.kvMap.object2ObjectEntrySet()) {
		entry.getKey().clear();
	...
	int size = 4;
	// kClass name
	size += (this.kClass.getName().length() * 2 + 4);
	// vClass name
	size += (this.vClass.getName().length() * 2 + 4);
	// Key + each array size
	ObjectIterator<Object2ObjectMap.Entry<K, V>> iterator = this.kvMap.object2ObjectEntrySet().fastIterator();
	while (iterator.hasNext()) {
	    Object2ObjectMap.Entry<K, V> entry = iterator.next();
	    size += (entry.getKey().getNumWriteBytes() + entry.getValue().getNumWriteBytes());
	}
	return size;
    }
	...
	int size = in.readInt();
	if (this.kvMap == null) {
	    this.kvMap = new Object2ObjectOpenHashMap<>(size);
	    this.kvMap.defaultReturnValue(null);
	}
	try {
	    this.kClass = Writable.forClass(in.readUTF());
	    this.vClass = Writable.forClass(in.readUTF());
	    for (int i = 0; i < size; i++) {
		K key = null;
		V val = null;
		if (freeKeys.isEmpty() && freeVals.isEmpty()) {
