        ...
        Tensor *selectedPosTensor, const Parameter &parameter)
{
    ...
    {
        DAAL_CHECK_STATUS(s, getUniformRandFrom0to1(selectedPos, selectedPosSize, parameter.engine));
    }
...
template<typename algorithmFPType, Method method, CpuType cpu>
Status PoolingKernel<algorithmFPType, method, cpu>::getUniformRandFrom0to1(int *uniformRand, const size_t nUniformRand, engines::EnginePtr engine)
{
    distributions::uniform::internal::UniformKernel<int, distributions::uniform::defaultDense, cpu> uniformKernel;
    distributions::uniform::Parameter<int> uniformParameter(0, data_feature_utils::internal::MaxVal<int, cpu>::get());
    uniformParameter.engine = engine;

    DAAL_CHECK(uniformKernel.compute(uniformParameter, nUniformRand, uniformRand), ErrorIncorrectErrorcodeFromGenerator);
    return Status();
}
