...
public class Serializer implements DataOutput {
    ...
    /**
     * Serialize on bytes which starts from bytes[0] to bytes[len - 1], with pos
     * between 0 ~ (len - 1)
     * 
     * @param bytes
     *            the byte[]
     ...
     * @param pos
     *            the current position in the byte[]
     */
    ...
    /**
     * Write the byte[] of the length beginning from the off to the output
     * stream
     */
    @Override
