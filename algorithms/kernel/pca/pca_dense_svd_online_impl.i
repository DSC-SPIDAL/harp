...
services::Status PCASVDOnlineKernel<algorithmFPType, cpu>::compute(InputDataType type,
    const NumericTablePtr &data,
    NumericTable &nObservations,
    NumericTable &auxiliaryTable,
    NumericTable &sumSVD,
    NumericTable &sumSquaresSVD)
{
    if(type == correlation)
...
template <typename algorithmFPType, CpuType cpu>
services::Status PCASVDOnlineKernel<algorithmFPType, cpu>::finalizeMerge(InputDataType type,
    const NumericTablePtr &nObservationsTable,
    ...
    DataCollectionPtr &rTables)
{
    ...
    if(s)
        s = this->scaleSingularValues(eigenvalues, nVectors);
    return s;
}

template <typename algorithmFPType, CpuType cpu>
services::Status PCASVDOnlineKernel<algorithmFPType, cpu>::decompose(const NumericTable *normalizedDataTable,
    NumericTable &auxiliaryTable)
{
    svd::Parameter kmPar;
    kmPar.leftSingularMatrix = svd::notRequired;
...
template <typename algorithmFPType, CpuType cpu>
inline void computeVariance(const size_t nObservations, const size_t nFeatures,
...
template <typename algorithmFPType, CpuType cpu>
services::Status PCASVDOnlineKernel<algorithmFPType, cpu>::normalizeDataset(const NumericTablePtr &data,
    size_t totalObservations, NumericTable &nObservations, NumericTable &sumSVD,
    NumericTable &sumSquaresSVD, NumericTablePtr& normalizedData)
{
    ...
    services::Status s;
    services::SharedPtr<HomogenNumericTableCPU<algorithmFPType, cpu>> normalized = HomogenNumericTableCPU<algorithmFPType, cpu>::create(nFeatures, nVectors, &s);
