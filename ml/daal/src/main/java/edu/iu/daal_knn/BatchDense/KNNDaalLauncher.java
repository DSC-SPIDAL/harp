...
public class KNNDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 8) {
      System.err
        .println("Usage: edu.iu.daal_svd.KNNDaalLauncher "
          + "<feature dim> "
          + "<file dim > "
	  + "<number of map tasks>"
	  + "<num threads> "
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runKNNMultiDense(nFeature, nFileDim, 
		    numMapTasks, numThreads, mem, trainDataDir, testDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total KNN Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runKNNMultiDense(
    ...
    Job KNNJob =
      configureKNNJob(nFeature, nFileDim,
		    numMapTasks, numThreads, mem, trainDataDir, testDataDir, dataDir, outDir, configuration);
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("KNN Job fails.");
    }
  }

  private Job configureKNNJob(
    int nFeature, int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, String testDataDir,
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "KNN_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(KNNDaalLauncher.class);
    job
      .setMapperClass(KNNDaalCollectiveMapper.class);
