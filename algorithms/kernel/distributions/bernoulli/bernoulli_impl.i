...
Status BernoulliKernel<algorithmFPType, method, cpu>::computeInt(int *resultArray, size_t n, const bernoulli::Parameter<algorithmFPType> &parameter)
{
    const int one  = 1;
    const int zero = 0;
    const size_t nElemsInBlock = 1024;

    UniformKernel<algorithmFPType, uniform::defaultDense, cpu> uniformKernel;
    uniform::Parameter<algorithmFPType> uniformParameter(0.0, 1.0);
    uniformParameter.engine   = parameter.engine;
    const algorithmFPType p   = parameter.p;
    ...
    for(size_t block = 0; block < nBlocks; block++)
    {
        ...
        DAAL_CHECK(uniformKernel.compute(uniformParameter, nElemsToProcess, buffer), ErrorIncorrectErrorcodeFromGenerator);
...
template<typename algorithmFPType, Method method, CpuType cpu>
Status BernoulliKernel<algorithmFPType, method, cpu>::computeFPType(NumericTable *resultTable, const bernoulli::Parameter<algorithmFPType> &parameter)
{
    ...
    if(nElemsInBlock > nCols)
    {
        ...
        for(size_t block = 0; block < nRowBlocks; block++) /* blocking by rows */
        {
            ...
            DAAL_CHECK(uniformKernel.compute(uniformParameter, nElemsToProcess, resultArray), ErrorIncorrectErrorcodeFromGenerator);
    ...
    else
    {
        ...
        for(size_t block = 0; block < nRows; block++) /* blocking by rows */
        {
            ...
            for(size_t colBlock = 0; colBlock < nColBlocks; colBlock++) /* blocking within one column */
            {
                ...
                DAAL_CHECK(uniformKernel.compute(uniformParameter, nElemsToProcess, array), ErrorIncorrectErrorcodeFromGenerator);
...
template<typename algorithmFPType, Method method, CpuType cpu>
Status BernoulliKernel<algorithmFPType, method, cpu>::compute(const bernoulli::Parameter<algorithmFPType> &parameter, NumericTable *resultTable)
{
      ...
      (*resultDict)[0].indexType      == data_feature_utils::DAAL_INT32_S)  /* if table is HomogenNT<int> */
    {
        ...
        return computeInt(resultArray, n, parameter);
    }
    else
    {
        return computeFPType(resultTable, parameter);
    }
    return Status();
}
