...
public class DynamicScheduler<I, O, T extends Task<I, O>> {
    ...
    protected static final Log LOG = LogFactory.getLog(DynamicScheduler.class);
	...
	if (input != null) {
	    inputQueue.add(new Input<I>(input, false, false));
	    if (isRunning) {
		inputCount++;
	    }
	}
     ...
     */
    public synchronized void submitAll(Collection<I> inputs) {
	for (I input : inputs) {
	    inputQueue.add(new Input<I>(input, false, false));
	}
	if (isRunning) {
	...
	for (int i = 0; i < inputs.length; i++) {
	    if (inputs[i] != null) {
		inputQueue.add(new Input<I>(inputs[i], false, false));
		submitCount++;
	    }
	}
	if (isRunning) {
	...
	if (isRunning && !isPausing) {
		...
		inputQueue.addLast(new Input<I>(null, true, false));
	    }
	    ComputeUtil.acquire(barrier1, numTaskMonitors);
	    inputCount -= inputQueue.size();
	}
    }
	...
	if (isRunning && !isPausing) {
	    ...
	    isPausing = true;
	    for (int i = 0; i < numTaskMonitors; i++) {
		inputQueue.addFirst(new Input<I>(null, true, false));
	    }
	    ComputeUtil.acquire(barrier1, numTaskMonitors);
	    inputCount -= inputQueue.size();
	}
    ...
    /**
     * Stop submission Send stop signal to the queue based on the number of
     * tasks
     */
	...
	if (isRunning) {
	    isRunning = false;
	    for (int i = 0; i < numTaskMonitors; i++) {
		inputQueue.addLast(new Input<I>(null, false, true));
	    }
	    for (int i = 0; i < numTaskMonitors; i++) {
    ...
    /**
     * Blocked and wait for outputs Invoke as while(hasOutput()) {
     * waitForOutput(); }
     * 
     * @return the output
     */
