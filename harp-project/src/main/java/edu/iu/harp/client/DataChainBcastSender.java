...
/*******************************************************
 * The actual sender for broadcasting the data using chain method We don't allow
 * the worker broadcasts to itself.
 ******************************************************/
public class DataChainBcastSender extends DataSender {
    /** Class logger */
    @SuppressWarnings("unused")
    private static final Logger LOG = Logger.getLogger(DataChainBcastSender.class);

    public DataChainBcastSender(Data data, Workers workers, byte command) {
	super(data, getDestID(workers.getSelfID(), workers.getNextID()), workers, command);
    }
     ...
     */
    private static int getDestID(int selfID, int nextID) {
	if (selfID == nextID) {
