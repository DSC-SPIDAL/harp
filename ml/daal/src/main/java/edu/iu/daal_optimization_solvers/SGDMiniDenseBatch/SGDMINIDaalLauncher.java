...
package edu.iu.daal_optimization_solvers.SGDMiniDenseBatch;
...
public class SGDMINIDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 11) {
      System.err
        .println("Usage: edu.iu.daal_svd.SGDMINIDaalLauncher "
          + "<file dim > "
	  + "<nfeature>"
	  + "<accuracyThreshold>"
	  + "<nIterations>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runSGDMINI(nFileDim, nFeatures, accuracyThreshold, nIterations, batchSize, learningRate, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total SGDMINI Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runSGDMINI(
    ...
    Job SGDMINIJob =
      configureSGDMINIJob(nFileDim, nFeatures, accuracyThreshold, nIterations, batchSize, learningRate,
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      SGDMINIJob.waitForCompletion(true);
    System.out
      .println("end job "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("SGDMINI Job fails.");
    }
  }

  private Job configureSGDMINIJob(
    ...
    Job job =
      Job
        .getInstance(configuration, "SGDMINI_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(SGDMINIDaalLauncher.class);
    job
      .setMapperClass(SGDMINIDaalCollectiveMapper.class);
