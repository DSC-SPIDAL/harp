...
package edu.iu.daal_kernel_func.RbfDenseBatch;
...
public class RbfDenseDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 9) {
      System.err
        .println("Usage: edu.iu.daal_svd.RbfDenseDaalLauncher "
          + "<feature dim> "
          + "<file dim > "
	  + "<sigma>"
	  + "<number of map tasks>"
    ...
    String leftDataDir = args[6];
    String rightDataDir = args[7];
    String workDir = args[8];
   
    launch(nFeature, nFileDim, sigma, numMapTasks, numThreads, mem, leftDataDir, rightDataDir, workDir);
    return 0;
  }

  private void launch(int nFeature, int nFileDim, double sigma,   
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runRbfDense(nFeature, nFileDim, sigma, 
		    numMapTasks, numThreads, mem, leftDataDir, rightDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total RbfDense Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runRbfDense(
    int nFeature, int nFileDim, double sigma, 
    int numMapTasks, int numThreads, int mem, 
    ...
    Job RbfDenseJob =
      configureRbfDenseJob(nFeature, nFileDim, sigma, 
		    numMapTasks, numThreads, mem, leftDataDir, rightDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      RbfDenseJob.waitForCompletion(true);
    System.out
      .println("end Jod "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("RbfDense Job fails.");
    }
  }

  private Job configureRbfDenseJob(
    int nFeature, int nFileDim, double sigma, 
    int numMapTasks, int numThreads, int mem, 
    String leftDataDir, String rightDataDir,
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "RbfDense_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(RbfDenseDaalLauncher.class);
    job
      .setMapperClass(RbfDenseDaalCollectiveMapper.class);
    ...
    jobConfig.setInt(Constants.FILE_DIM,
      nFileDim);
    jobConfig.setDouble(Constants.SIGMA,
      sigma);
