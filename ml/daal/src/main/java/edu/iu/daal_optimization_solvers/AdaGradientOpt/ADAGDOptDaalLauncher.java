...
public class ADAGDOptDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 11) {
      System.err
        .println("Usage: edu.iu.daal_svd.ADAGDOptDaalLauncher "
          + "<file dim > "
	  + "<nfeature>"
	  + "<accuracyThreshold>"
	  + "<nIterations>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runADAGDOpt(nFileDim, nFeatures, accuracyThreshold, nIterations, batchSize, learningRate, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total ADAGDOpt Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runADAGDOpt(
    ...
    Job ADAGDOptJob =
      configureADAGDOptJob(nFileDim, nFeatures, accuracyThreshold, nIterations, batchSize, learningRate,
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      ADAGDOptJob.waitForCompletion(true);
    System.out
      .println("end job "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("ADAGDOpt Job fails.");
    }
  }

  private Job configureADAGDOptJob(
    ...
    Job job =
      Job
        .getInstance(configuration, "ADAGDOpt_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(ADAGDOptDaalLauncher.class);
    job
      .setMapperClass(ADAGDOptDaalCollectiveMapper.class);
