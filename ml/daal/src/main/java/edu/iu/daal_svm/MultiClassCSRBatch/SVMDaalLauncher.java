...
public class SVMDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 9) {
      System.err
        .println("Usage: edu.iu.daal_svd.SVMDaalLauncher "
	  + "<nClass>"
	  + "<number of map tasks>"
	  + "<num threads> "
          + "<mem>"
          + "<input train dir>"
          + "<input train truth dir>"
          + "<input test dir>"
          + "<input test truth dir>"
          + "<work dir>");
      ToolRunner
        .printGenericCommandUsage(System.err);
      return -1;
    }
    int nClass = Integer.parseInt(args[0]);
    int numMapTasks = Integer.parseInt(args[1]);
    int numThreads = Integer.parseInt(args[2]);
    int mem = Integer.parseInt(args[3]);
    String trainDataDir = args[4];
    String trainTruthDataDir = args[5];
    String testDataDir = args[6];
    String testTruthDataDir = args[7];
    String workDir = args[8];
   
    launch(nClass, numMapTasks, numThreads, mem, trainDataDir, trainTruthDataDir, testDataDir, testTruthDataDir, workDir);
    return 0;
  }

  private void launch(int nClass, 
		      int numMapTasks, int numThreads, int mem, 
		      String trainDataDir, String trainTruthDataDir, String testDataDir, String testTruthDataDir, String workDir) throws IOException,
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runSVMMultiCSR(nClass, numMapTasks, numThreads, mem, trainDataDir, trainTruthDataDir, testDataDir, testTruthDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total SVM Multi CSR Execution Time: "
        + (endTime - startTime));
  }

  private void runSVMMultiCSR(
    int nClass, 
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, String trainTruthDataDir, String testDataDir,
    String testTruthDataDir,
    Path dataDir,
    ...
    Job svmJob =
      configureSVMJob(nClass, 
		    numMapTasks, numThreads, mem, trainDataDir, trainTruthDataDir, testDataDir, testTruthDataDir, dataDir, outDir, configuration);
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("SVM Job fails.");
    }
  }
  ...
  private Job configureSVMJob(
    ...
    Job job =
      Job
        .getInstance(configuration, "svm_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(SVMDaalLauncher.class);
    job
      .setMapperClass(SVMDaalCollectiveMapper.class);
    ...
    jobConfig.set(Constants.TRAIN_TRUTH,
      trainTruthDataDir);
    jobConfig.set(Constants.TEST_FILE_PATH,
      testDataDir);
    jobConfig.set(Constants.TEST_TRUTH,
      testTruthDataDir);
