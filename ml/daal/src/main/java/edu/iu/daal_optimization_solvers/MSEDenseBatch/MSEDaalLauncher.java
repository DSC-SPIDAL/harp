...
public class MSEDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 7) {
      System.err
        .println("Usage: edu.iu.daal_svd.MSEDaalLauncher "
          + "<file dim > "
          + "<feature dim > "
	  + "<number of map tasks>"
	  + "<num threads> "
    ...
    launch(nFileDim, nFeatures, numMapTasks, numThreads, mem, trainDataDir, workDir);

    return 0;
  }

  private void launch(int nFileDim, int nFeatures,
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runMSE(nFileDim, nFeatures, 
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total MSE Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runMSE(
    int nFileDim, int nFeatures,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job MSEJob =
      configureMSEJob(nFileDim, nFeatures,
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("MSE Job fails.");
    }
  }

  private Job configureMSEJob(
    int nFileDim, int nFeatures,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "MSE_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(MSEDaalLauncher.class);
    job
      .setMapperClass(MSEDaalCollectiveMapper.class);
