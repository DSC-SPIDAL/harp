...
public class ODMultiDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 6) {
      System.err
        .println("Usage: edu.iu.daal_svd.ODMultiDaalLauncher "
          + "<file dim > "
	  + "<number of map tasks>"
	  + "<num threads> "
          + "<mem>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    System.out
      .println("Total ODMulti Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runODMulti(
    int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      ODMultiJob.waitForCompletion(true);
    System.out
      .println("end JODMulti "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
            .getTime()));
    System.out
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("ODMulti Job fails.");
    }
  }

  private Job configureODMultiJob(
    int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "ODMulti_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(ODMultiDaalLauncher.class);
    job
      .setMapperClass(ODMultiDaalCollectiveMapper.class);
