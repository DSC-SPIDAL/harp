...
Status daal::data_management::NumericTable::allocateBasicStatistics()
{
    ...
    if(basicStatistics.get(NumericTable::minimum).get() == NULL || basicStatistics.get(NumericTable::minimum)->getNumberOfColumns() != ncols)
    {
        basicStatistics.set(NumericTable::minimum,
            NumericTablePtr(new HomogenNumericTable<DAAL_SUMMARY_STATISTICS_TYPE>(ncols, 1, NumericTableIface::doAllocate)));
    }
...
Status createSparseTableImpl(const NumericTablePtr &inTable, CSRNumericTablePtr &resTable)
{
    ...
    resTable.reset(new CSRNumericTable(resData, resColIndices, resRowIndices, nFeatures, nObservations));
    DAAL_CHECK(resTable.get(), ErrorMemoryAllocationFailed);
