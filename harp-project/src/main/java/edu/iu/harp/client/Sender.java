...
public abstract class Sender {
    ...
    /**
     * To send data between workers, sender and receiver must both have valid
     * IDs.
     * 
     * @param d
     *            the data
    ...
    /**
     * Use host and port directly do send between any to processes It can send
     * data from a process other than workers or send data from a worker to a
     * process outside.
     * 
     * If send data from outside, data's worker ID should be unknown
     * 
     * @param d
     * @param h
     * @param p
     * @param command
     */
    public Sender(Data d, String h, int p, byte command) {
	data = d;
	destWorkerID = Constant.UNKNOWN_WORKER_ID;
	workers = null;
	host = h;
	...
	if (data == null || host == null || port == Constant.UNKNOWN_PORT || commandType == Constant.UNKNOWN_CMD) {
	    return false;
	}
	...
	// Open connection
	Connection conn = Connection.create(host, port, true);
	if (conn == null) {
     ...
     * @throws Exception
     */
    protected abstract void handleData(final Connection conn, final Data data) throws Exception;
}
