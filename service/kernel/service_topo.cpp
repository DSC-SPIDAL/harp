 ...
 */
static int __internal_daal_bindContext(unsigned int cpu)
{
    int ret = -1;
#ifdef __linux__
...
unsigned  int lcl_OSProcessorCount = 0;
    ...
    {   lcl_OSProcessorCount = 0;
        ...
        if (!GetLogicalProcessorInformationEx(RelationGroup, pSystem_rel_info, &cnt)) { glbl_obj.error |= _MSGTYP_UNKNOWNERR_OS; return 0; }
        if( pSystem_rel_info->Relationship != RelationGroup) {   glbl_obj.error |= _MSGTYP_UNKNOWNERR_OS; return 0;  }
...
{unsigned int i, sum = 0;
    ...
    {   cnt = BLOCKSIZE_4K;
        ...
        {   glbl_obj.error |= _MSGTYP_UNKNOWNERR_OS; return ;
        }
        ...
        for (i = 0; i < grpCnt; i ++)
        {
            if( !GetProcessGroupAffinity(GetCurrentProcess(), &grpCnt, &grpAffinity[0]) )
...
{   unsigned long SMTMaxCntPerEachCache;
    CPUIDinfo info;
        ...
        else  // no support for __internal_daal_cpuid leaf 4 and no HT
 ...
 */
unsigned _internal_daal_FreeArrays(void)
{

    _INTERNAL_DAAL_FREE(glbl_obj.pApicAffOrdMapping);

    _INTERNAL_DAAL_FREE(glbl_obj.perPkg_detectedCoresCount.data);

    _INTERNAL_DAAL_FREE(glbl_obj.perCore_detectedThreadsCount.data);

    _INTERNAL_DAAL_FREE(glbl_obj.perCache_detectedCoreCount.data);

    _INTERNAL_DAAL_FREE(glbl_obj.perEachCache_detectedThreadCount.data);

    _INTERNAL_DAAL_FREE(glbl_obj.cpuid_values);
...
static int __internal_daal_allocArrays(unsigned  cpus)
{
    ...
    i = cpus+1;
    glbl_obj.pApicAffOrdMapping =
        (idAffMskOrdMapping_t *)_INTERNAL_DAAL_MALLOC(i*sizeof(idAffMskOrdMapping_t));
...
{   unsigned  i;
    ...
    for (i=0; i < glbl_obj.OSProcessorCount;i++) {
        ...
        if(__internal_daal_testGenericAffinityBit(&glbl_obj.cpu_generic_processAffinity, i) == 1) {
            ...
            if( __internal_daal_bindContext(i) ) {
                glbl_obj.error |= _MSGTYP_UNKNOWNERR_OS;
                break;
            }
...
{
    if (!glbl_obj.EnumeratedPkgCount)
        __internal_daal_buildSystemTopologyTables();
}
...
{
    if (!glbl_obj.EnumeratedPkgCount)      __internal_daal_initCpuTopology();
...
{
    if (!glbl_obj.EnumeratedEachCacheCount[subleaf])       __internal_daal_initCpuTopology();
...
{
    if (!glbl_obj.EnumeratedCoreCount)     __internal_daal_initCpuTopology();
