...
public class ODUniDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 6) {
      System.err
        .println("Usage: edu.iu.daal_svd.ODUniDaalLauncher "
          + "<file dim > "
	  + "<number of map tasks>"
	  + "<num threads> "
          + "<mem>"
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    System.out
      .println("Total ODUni Uni Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runODUni(
    int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      ODUniJob.waitForCompletion(true);
    System.out
      .println("end JOB DUni "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
            .getTime()));
    System.out
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("ODUni Job fails.");
    }
  }

  private Job configureODUniJob(
    int nFileDim,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "ODUni_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(ODUniDaalLauncher.class);
    job
      .setMapperClass(ODUniDaalCollectiveMapper.class);
