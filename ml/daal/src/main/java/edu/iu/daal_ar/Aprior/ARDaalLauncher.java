...
public class ARDaalLauncher extends Configured
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    System.out
      .println("Total AR Multi Dense Execution Time: "
        + (endTime - startTime));
  }
  ...
  private void runARMultiDense(
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("AR Job fails.");
    }
  }
  ...
  private Job configureARJob(
    ...
    Job job =
      Job
        .getInstance(configuration, "AR_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(ARDaalLauncher.class);
    job
      .setMapperClass(ARDaalCollectiveMapper.class);
