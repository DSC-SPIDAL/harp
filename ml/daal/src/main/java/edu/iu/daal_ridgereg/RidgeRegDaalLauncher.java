...
public class RidgeRegDaalLauncher extends Configured
implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (args.length < 8) {
      System.err.println("Usage: edu.iu.daal_ridgereg"
                + "<input train dir> "
                + "<input test dir>"
...
ExecutionException, ClassNotFoundException {
  ...
  runRidgeReg(inputDir, testDirPath, testGroundTruthDirPath, mem, batchSize, numMapTasks,
      numThreadsPerWorker, modelDir,
      outputDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total Ridge Regression Execution Time: "
        + (endTime - startTime));
}

private void runRidgeReg(Path inputDir, String testDirPath, String testGroundTruthDirPath, int mem, int batchSize,  
    ...
    Job ridgeRegJob =
      configureRidgeRegJob(inputDir, testDirPath, testGroundTruthDirPath, mem, batchSize,
        numMapTasks, numThreadsPerWorker,
        modelDir, outputDir, configuration);

    boolean jobSuccess =
      ridgeRegJob.waitForCompletion(true);
  ...
  private Job configureRidgeRegJob(Path inputDir,
    ...
    Job job = Job.getInstance(configuration, "ridgereg_job");
    ...
    job.setJarByClass(RidgeRegDaalLauncher.class);
    job.setMapperClass(RidgeRegDaalCollectiveMapper.class);
