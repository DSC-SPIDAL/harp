...
public class DecisionTree {
  /** Instead of checking each index we'll skip every INDEX_SKIP indices unless there's less than MIN_SIZE_TO_CHECK_EACH*/
	...
	private static final int MIN_NODE_SIZE=RandomForestConstants.MIN_NODE_SIZE;	

	private static final int MAX_DEPTH=RandomForestConstants.MAX_DEPTH;	
	...
	*/
    ...
    // System.out.println("Just entered the function to create the DT (DecisionTree.java ln 59)....");

		ArrayList<ArrayList<String>> train = new ArrayList<ArrayList<String>>(N);
		ArrayList<ArrayList<String>> test = new ArrayList<ArrayList<String>>();

    System.out.println("(DecisionTree.java ln 64) Boot strapping the data for this tree...");
		BootStrapSample(data,train,test,treenum);
		testN=test.size();
		correct=0;
		// System.out.println("Starting to create the root of a tree (DecisionTree.java ln 59)");
		root=CreateTree(train,treenum);
    // System.out.println("(DecisionTree.java ln 64) just created the root, so flushing the data...");
...
//		System.out.println("bootstrap N:"+N+" size of bootstrap:"+bootstrap.size());
	...
	private TreeNode CreateTree(ArrayList<ArrayList<String>> train, int ntree){
		...
		root.label = "|ROOT|";
		root.data=train;
		System.out.println("creating the decision tree with call to RecursiveSplit (DecisionTree.java ln 109)");
		RecursiveSplit(root,ntree);
    System.out.println("The tree number " + ntree + " has been learned.");
		return root;
	}
	/**
	 *
...
//							System.out.println("going in child :"+evalNode.label);
	...
	private void RecursiveSplit(TreeNode parent, int Ntreenum){
      ...
      System.out.println("The current parent in RecursiveSplit (DecisionTree.java ln 312) " + parent);
			//-------------------------------Step A
      // Check if all of the data points that have made it down the given
      // branch belong to the same class. If they do, then this is a terminal
      // node and not more splitting should happen.
			...
			//-------------------------------Step B
      // The number of data points that have made it down this branch
      ...
      // The randomly select attributes that are to be included as candidate
      ...
      // System.out.println("--------------------------");
      // System.out.println("(DecisionTree.java ln 333) The attributes that were randomly selected to be considered in this split " + vars);
      // System.out.println("--------------------------");
      ...
      int counter = 0;
        ...
        counter ++;
        // The data points are subselected in that only those feature values
        // that can discriminate between classes. All others are ignored because
        // they are not meaningful for discriminating between classes.
				ArrayList<Integer> DataPointToCheck=new ArrayList<Integer>();// which data points to be scrutinized
        // System.out.println("--------------------------");
        // System.out.println("(DecisionTree.java ln 340) The data is being sorted according to the attribute: " + m);
        // System.out.println(counter + " out of " + vars.size());
        // System.out.println("--------------------------");
        // Sort the data matrix on the attribute m so that the meaningful
        // attribute values w.r.t. the class can be found
				SortAtAttribute(parent.data, m);//sorts on a particular column in the row
        // System.out.println("(DecisionTree.java ln 356) Data sorted on " + m);
        //System.out.println("Sorted data " + parent.data);
        // Compare the classes of subsequent data points to find possible places to
        ...
        //System.out.println("(DecisionTree.java ln 366) Data points to check for split point " + DataPointToCheck);
        //System.out.println("(DecisionTree.java ln 366) Data points to check for split point found.");
        ...
        //System.out.println("(DecisionTree.java ln 373) Checked that not all of the data points have the same class.");
				if (DataPointToCheck.size() > MIN_SIZE_TO_CHECK_EACH){
          //System.out.println("(DecisionTree.java ln 375) There are " + DataPointToCheck.size() + " different data points to check.");
          //System.out.println("(DecisionTree.java ln 376) The minimum is " + MIN_SIZE_TO_CHECK_EACH);
					for (int i=0;i<DataPointToCheck.size();i+=INDEX_SKIP){
            //System.out.println("(DecisionTree.java ln 375) Check the value of splitting on " + i);
						CheckPosition(m, DataPointToCheck.get(i), Nsub, lowestE, parent, Ntreenum);
            //System.out.println("(DecisionTree.java ln 379) The value of this split " + lowestE.d);
						if (lowestE.d == 0)//lowestE now has the minimum conditional entropy so IG is max there
              // System.out.println("(DecisionTree.java ln 388) A lowestE.d of 0 is found.");
              break;
					}
				}else{
          // System.out.println("(DecisionTree.java ln 384) There is not more than the minimum number of points to check");
					for (int k:DataPointToCheck){
            //System.out.println("(DecisionTree.java ln 387) Check the value of splitting on " + k);
						CheckPosition(m,k, Nsub, lowestE, parent, Ntreenum);
            //System.out.println("(DecisionTree.java ln 388) The value of this split " + lowestE.d);
						if (lowestE.d == 0)//lowestE now has the minimum conditional entropy so IG is max there
              // System.out.println("(DecisionTree.java ln 388) A lowestE.d of 0 is found.");
							break;
					}
				}
        //System.out.println("(DecisionTree.java ln 387) lowestE.d value " + lowestE.d);
				if (lowestE.d == 0)
					break;
        //System.out.println("(DecisionTree.java ln 389) Found split val.");
      }
			...
			for(TreeNode Child:parent.ChildNode){
				...
				if(!Child.isLeaf){
          System.out.println("DecisionTree.java ln 411) Moving onto the next level...");
    ...
    // System.out.println("(DecisionTree.java ln 405) Feature Type: " + forest.DataAttributes.get(m));
		// if(forest.DataAttributes.get(m) == 'C')
		// 	System.out.print("");//Collections.sort(data,new AttributeComparatorCateg(m));
		// else
  //     System.out.println("(DecisionTree.java ln 409) Sorting the data on the attribute " + m);
	...
	private class AttributeComparatorReal implements Comparator<ArrayList<String>>{
      ...
      //System.out.println("Value comparison for attributed " + m + " between " + a2 + " " + b2);
			if(a2<b2)
				return -1;
			else if(a2>b2)
				return 1;
	...
	private double CheckPosition(int m,int n,int Nsub,DoubleWrap lowestE,TreeNode parent, int nTre){
		...
		if(forest.DataAttributes.get(m)=='C'){
			...
			for(String S1:uni_classes){
				...
				// System.out.println("Mapping Class: "+S1+" to attribute: "+Resul);
			}
			//calculating entropy
