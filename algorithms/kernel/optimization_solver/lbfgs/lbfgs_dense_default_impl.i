    ...
    for (; t < nLIterations;)
    {
        for(; epoch < (t + 1) * L; ++epoch, ++curIteration)
        {
            ...
            DAAL_CHECK_STATUS(s, task.updateArgument(curIteration, t, epoch, m, correctionIndex, nTerms, batchSize,
                accuracyThreshold, gradientFunction, ntGradient, argument, bContinue));
            if(!bContinue)
            {
        ...
        if (t >= 2)
        {
            ...
            DAAL_CHECK_STATUS(s, task.updateCorrectionPairBatchIndices(iPredefinedIndicesCorrectionRow, nTerms, correctionPairBatchSize));
            ...
            services::SharedPtr<daal::internal::HomogenNumericTableCPU<int, cpu> > &ntBatchIndices)
{
    ...
    {
        DAAL_CHECK(_rng.uniformWithoutReplacement(batchSize, batchIndices, _baseGen, 0, (int)nTerms) == 0,
    ...
    NumericTable *correctionPairsResult, size_t nTerms, size_t batchSize, size_t correctionPairBatchSize)
{
    ...
    argumentLCurTable.reset(new HomogenNumericTableCPU<algorithmFPType, cpu>(argumentLCur, 1, this->argumentSize));
    ...
    return initRngState(optionalArgumentInput, parameter);
}
    ...
    OptionalArgument *optionalArgumentResult, size_t nIterations, size_t epoch, size_t corrIndex)
{
    ...
    DAAL_CHECK(_baseGen.saveState(pState->get()) == 0, ErrorIncorrectErrorcodeFromGenerator);
    return s;
}
...
template<typename algorithmFPType, CpuType cpu>
Status LBFGSTask<algorithmFPType, cpu>::initRngState(OptionalArgument *optionalArgumentInput, const Parameter* parameter)
{
    ...
    if(!pState || !pState->size())
        return Status();
    DAAL_CHECK(_baseGen.loadState(pState->get()) == 0, ErrorIncorrectErrorcodeFromGenerator);
    return Status();
}
            ...
            services::SharedPtr<daal::internal::HomogenNumericTableCPU<int, cpu> > &ntIndices)
{
    ...
    ntIndices.reset(new HomogenNumericTableCPU<int, cpu>(indices, size, 1));
