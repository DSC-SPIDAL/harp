...
template<typename T>
inline services::Status pickElementsRaw(const FeatureIndicesIfacePtr &indices,
                                        T *elements, T **pickedElements)
{
    DAAL_ASSERT( indices );
...
template<typename T>
inline services::internal::CollectionPtr<T *> pickElements(const FeatureIndicesIfacePtr &indices, T *elements,
                                                           services::Status *status = NULL)
{
    DAAL_ASSERT( indices );
    DAAL_ASSERT( elements );

    services::internal::CollectionPtr<T *> pickedElements =
        services::internal::HeapAllocatableCollection<T *>::create(indices->size(), status);
...
inline services::internal::CollectionPtr<T *> pickElements(const FeatureIndicesIfacePtr &indices,
                                                           const services::Collection<T> &elements,
                                                           services::Status *status = NULL)
{
...
public:
    ...
    const services::internal::CollectionPtr<T> &getElements() const
    {
        return _elements;
    }

    const services::internal::CollectionPtr<T *> &getPickedElements() const
    {
