...
services::Status PCACorrelationKernel<batch, algorithmFPType, cpu>::compute(
    const data_management::NumericTablePtr& pData, bool isCorrelation,
    const BatchParameter<algorithmFPType, correlationDense> *parameter,
    data_management::NumericTable& eigenvectors,
    data_management::NumericTable& eigenvalues)
{
    if(isCorrelation)
        return this->computeCorrelationEigenvalues(*pData, eigenvectors, eigenvalues);
    ...
    if(s)
        s = this->computeCorrelationEigenvalues(*parameter->covariance->getResult()->get(covariance::covariance),
