...
/*******************************************************
 * The information of the workers. Not modifiable from outside
 ******************************************************/
public class Workers extends Nodes {
    ...
    /**
     * Initialization the workers. Assign IDs to workers; Master is the worker
     * with ID 0;
     * 
     * @param reader
     *            the BufferedReader
	...
	workerInfos = new ConcurrentHashMap<>(initCapacity);
	rackWorkers = new ConcurrentHashMap<>(initCapacity);
	Map<Integer, List<String>> nodes = this.getNodes();
	...
	for (int rackID : getRackList()) {
		...
		workerInfos.put(workerID, new WorkerInfo(workerID, node, workerPortBase + workerID, rackID));
     ...
     * 
     * @return true if this is the only worker, false otherwise
     */
     ...
     * 
     * @return true if this worker is in the workers, false otherwise
     */
