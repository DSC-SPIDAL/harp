...
protected:
    ...
    void computeGHSums2(IndexType iFeature, const IndexType* aIdx,
...
/*
one-hot encoding, only two values, 0/1
*/
template<typename algorithmFPType, CpuType cpu>
void TreeBuilderIndexed<algorithmFPType, cpu>::computeGHSums2(IndexType iFeature, const IndexType* aIdx,
    const typename super::SplitJobType& job, typename super::ghSumType* aGHSum, algorithmFPType& gTotal, algorithmFPType& hTotal) const
{
...
//        }
        ...
        //two ordered lists search
        {
            ...
            while(i < n){
                ...
                if (indexedFeature[j] == IndexType(-1)) break;

                if (aIdx[i] > indexedFeature[j]){
                    //move index forward
                    j++;
                }
                else if(aIdx[i] < indexedFeature[j]){
                    i++;
                }
                else{
                    //match
        ...
        auto& sum = aGHSum[0];
        sum.n = n - aGHSum[1].n;
        sum.g = gTotal - aGHSum[1].g;
        sum.h = hTotal - aGHSum[1].h;

        //std::cout << realcnt << ":" << n <<" ";
        //std::cout << disordercnt << ":" << n <<" ";
        return;
    }
    ...
    if(_ctx.aSampleToF())
    {
        ...
        const IndexType* aSampleToF = _ctx.aSampleToF();
        {
            ...
            while(i < n){
                ...
                if (iRow > indexedFeature[j]){
                    //move index forward
                    j++;
                }
                else if(iRow < indexedFeature[j]){
                    i++;
                }
                else{
                    //match
    ...
    else
    {
        ...
        //}
        {
            ...
            while(i < n){
                ...
                if (indexedFeature[j] == IndexType(-1)) break;
                const IndexType iSample = aIdx[i];
                if (iSample > indexedFeature[j]){
                    //move index forward
                    j++;
                }
                else if(iSample < indexedFeature[j]){
                    i++;
                }
                else{
                    //match
    ...
    const typename super::SplitJobType& job, typename super::SplitDataType& split, bool bUpdateWhenTie) const
{
    ...
    if (nDiffFeatMax == 2){
        //std::cout << ".";
        //computeGHSums2(iFeature, aIdx, job, aGHSum, gTotal, hTotal);
