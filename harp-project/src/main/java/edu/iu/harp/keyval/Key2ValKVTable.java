...
/*******************************************************
 * Key2ValKVPartitionCombiner defines how to merge Key2ValKVPartitions
 ******************************************************/
class Key2ValKVPartitionCombiner<K extends Key, V extends Value, P extends Key2ValKVPartition<K, V>>
    ...
    Key2ValKVPartitionCombiner(ValCombiner<V> combiner) {
	...
	Object2ObjectOpenHashMap<K, V> nMap = np.getKVMap();
	ObjectIterator<Object2ObjectMap.Entry<K, V>> iterator = nMap.object2ObjectEntrySet().fastIterator();
	List<K> rmKeys = new LinkedList<>();
	while (iterator.hasNext()) {
	    Object2ObjectMap.Entry<K, V> entry = iterator.next();
	    K key = entry.getKey();
	    V val = entry.getValue();
	    ValStatus status = op.putKeyVal(key, val, valCombiner);
	    if (status == ValStatus.ADDED) {
		rmKeys.add(key);
		if (!op.getFreeKeys().isEmpty()) {
		    np.getFreeKeys().add(op.getFreeKeys().remove(0));
		}
...
public abstract class Key2ValKVTable<K extends Key, V extends Value, P extends Key2ValKVPartition<K, V>>
    ...
    private static final Logger LOG = Logger.getLogger(Key2ValKVTable.class);
    ...
    public Key2ValKVTable(int tableID, ValCombiner<V> combiner, Class<K> kClass, Class<V> vClass, Class<P> pClass) {
	super(tableID, new Key2ValKVPartitionCombiner<K, V, P>(combiner));
	this.kClass = kClass;
	this.vClass = vClass;
	this.pClass = pClass;
	this.valCombiner = combiner;
     ...
     * @return the ValStatus
     */
    private ValStatus addKVInPartition(P partition, K key, V val) {
