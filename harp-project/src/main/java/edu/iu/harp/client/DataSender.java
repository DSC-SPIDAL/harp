...
/*******************************************************
 * The actual sender for sending the data. Different from DataReceiver, we don't
 * release the encoded bytes in failures.
 ******************************************************/
public class DataSender extends Sender {
    ...
    public DataSender(Data data, int destWorkerID, Workers workers, byte command) {
	super(data, destWorkerID, workers, command);
    }

    public DataSender(Data data, String host, int port, byte command) {
     ...
     */
    @Override
    protected void handleData(final Connection conn, final Data data) throws Exception {
	// Get head size and body size
	int headArrSize = getHeadSize(data);
	ByteArray opArray = getOPByteArray(headArrSize);
	if (opArray == null) {
	    throw new IOException("Cannot get op array.");
	}
	try {
     ...
     * @throws IOException
     */
    private void sendBodyBytes(final OutputStream out, final Data data) throws IOException {
	// Send content data, check the array size
	// first. Sending or receiving null array is
	// allowed
	DataStatus bodyStatus = data.getBodyStatus();
	if (bodyStatus == DataStatus.ENCODED_ARRAY_DECODED || bodyStatus == DataStatus.ENCODED_ARRAY
		|| bodyStatus == DataStatus.ENCODED_ARRAY_DECODE_FAILED) {
	    ByteArray bodyArray = data.getBodyArray();
	    IOUtil.sendBytes(out, bodyArray.get(), bodyArray.start(), bodyArray.size());
	}
    }
}
