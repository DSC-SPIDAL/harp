...
protected:
    ...
    Status setWeightsAndBiasesFromTable(ForwardLayersPtr &forwardLayers)
    {
        ...
        for (size_t i = 0; i < _nLayers; i++)
        {
            ...
            TensorPtr weights(new HomogenTensor<modelFPType>(_wDimsCollection[i], wbArray + _wOffsets[i]));
            DAAL_CHECK_MALLOC(weights.get())
            TensorPtr biases(new HomogenTensor<modelFPType>(_bDimsCollection[i], wbArray + _bOffsets[i]));
            DAAL_CHECK_MALLOC(biases.get())
...
protected:
    ...
    Status setWeightsAndBiasesFromTable(BackwardLayersPtr &backwardLayers)
    {
        ...
        for (size_t i = 0; i < _nLayers; i++)
        {
            ...
            TensorPtr weightDerivatives(new HomogenTensor<modelFPType>(_wDimsCollection[i], wbArray + _wOffsets[i]));
            DAAL_CHECK_MALLOC(weightDerivatives.get())
            TensorPtr biasDerivatives(new HomogenTensor<modelFPType>(_bDimsCollection[i], wbArray + _bOffsets[i]));
            DAAL_CHECK_MALLOC(biasDerivatives.get())
