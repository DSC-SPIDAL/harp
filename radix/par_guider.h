...
class ParGuider {
public:
  ParGuider(int log_width_bytes, int thread_id, int num_threads,
            const RGraph &g) :
      num_bins_(CalcNumBins(log_width_bytes, g)), bins_(num_bins_),
      indices_(num_bins_), buffer_tails_(num_bins_), vert_starts_(num_threads+1) {
    shamt_ = 0;
    ...
    size_t buffer_size_bytes = num_bins_ * kBufferSize_*sizeof(Payload_);
    buffer_ = (Payload_*) aligned_alloc(64, buffer_size_bytes);
    for (int b=0; b < num_bins_; b++) {
      bins_[b].resize(bin_sizes[b]);
      indices_[b].reserve(bin_sizes[b]);
      TouchMem(bins_[b]);
      bins_[b].clear();
      buffer_tails_[b] = b * kBufferSize_;
    }
    FillIndicies(thread_id, num_threads, g);
  }
  ...
  void SendMsg(const NodeID_ dest, const Payload_ data) {
    ...
    buffer_[buffer_tails_[b]++] = data;
    if (buffer_tails_[b] == (b+1)*kBufferSize_) {
      const size_t copy_size_bytes = kBufferSize_ * sizeof(Payload_);
  ...
  pvector<Payload_>& GetBin(int b) {
    return bins_[b];
  }

  pvector<NodeID_>& GetIndex(int b) {
    return indices_[b];
  }
...
private:
  ...
  pvector<pvector<Payload_>> bins_;
  pvector<pvector<NodeID_>> indices_;
  Payload_* buffer_;
  ...
  void FillIndicies(int thread_id, int num_threads, const RGraph &g) {
    NodeID_ u = range_start(thread_id, num_threads, g);
    NodeID_ u_end = range_end(thread_id, num_threads, g);
    for (; u < u_end; u++) {
      for (NodeID_ v : g.out_neigh(u)) {
        indices_[CalcBin(v)].push_back(v);
      }
    }
  }
  ...
  void TouchMem(pvector<Payload_> &bin) {
    const size_t stride = 4096 / sizeof(Payload_);
