...
public class StaticScheduler<I, O, T extends Task<I, O>> {
    protected static final Log LOG = LogFactory.getLog(StaticScheduler.class);
     ...
     *            the input
     */
    public synchronized void submit(int taskID, I input) {
	...
	if (isRunning && !isPausing) {
		...
		taskMonitor.submit(new Input<I>(null, true, false));
	    }
	...
	if (isRunning) {
	    isRunning = false;
	    for (TaskMonitor<I, O, T> taskMonitor : taskMonitors) {
		taskMonitor.submit(new Input<I>(null, false, true));
	    }
    ...
    /**
     * 
     * Blocked and wait for output. Invoke as while(hasOutput()) {
     * waitForOutput(); }
     * 
     * @return the output
     */
