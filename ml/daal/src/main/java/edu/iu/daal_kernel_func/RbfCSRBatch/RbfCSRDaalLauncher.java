...
package edu.iu.daal_kernel_func.RbfCSRBatch;
...
public class RbfCSRDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 7) {
      System.err
        .println("Usage: edu.iu.daal_svd.RbfCSRDaalLauncher "
	  + "<sigma>"
	  + "<number of map tasks>"
	  + "<num threads> "
          + "<mem>"
    ...
    String leftDataDir = args[4];
    String rightDataDir = args[5];
    String workDir = args[6];
   
    launch(sigma, numMapTasks, numThreads, mem, leftDataDir, rightDataDir, workDir);
    return 0;
  }

  private void launch(double sigma,   
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    System.out
      .println("Total RbfCSR Execution Time: "
        + (endTime - startTime));
  }

  private void runRbfCSR(
    double sigma, 
    int numMapTasks, int numThreads, int mem, 
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      RbfCSRJob.waitForCompletion(true);
    System.out
      .println("end Jod "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("RbfCSR Job fails.");
    }
  }

  private Job configureRbfCSRJob(
    double sigma, 
    int numMapTasks, int numThreads, int mem, 
    String leftDataDir, String rightDataDir,
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "RbfCSR_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(RbfCSRDaalLauncher.class);
    job
      .setMapperClass(RbfCSRDaalCollectiveMapper.class);
