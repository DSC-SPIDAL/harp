...
Status QRDistributedStep2Kernel<algorithmFPType, method, cpu>::compute(const size_t na, const NumericTable *const *a,
                                                                          const size_t nr, NumericTable *r0, NumericTable *r[], const daal::algorithms::Parameter *par,
                                                                          data_management::KeyValueDataCollection *inCollection)
{
    ...
    const int ec = compute_QR_on_one_node<algorithmFPType, cpu>( nxb, n, Aux2T, nxb, RT, n );
    if(ec)
        return Status(services::ErrorID(ec));
 ...
 */
template <typename algorithmFPType, daal::algorithms::qr::Method method, CpuType cpu>
Status QRDistributedStep3Kernel<algorithmFPType, method, cpu>::compute(const size_t na, const NumericTable *const *a,
