...
services::Status Result::setResultForBackward(const daal::algorithms::Input *input)
{
    ...
    else
    {
        ...
        NumericTablePtr numberOfCoefficients(new HomogenNumericTable<int>(
            1, 1, NumericTable::doAllocate, (int)numberOfCoefficientsTableValue));
        DAAL_CHECK_MALLOC(numberOfCoefficients);
