...
public class DFREGTAVDaalLauncher extends Configured
  implements Tool {
  ...
  public int run(String[] args) throws Exception {
      ...
      if (args.length < 8) {
      System.err
        .println("Usage: edu.iu.daal_svd.DFREGTAVDaalLauncher "
          + "<feature dim> "
          + "<file dim > "
	  + "<ntrees>"
	  + "<number of map tasks>"
    ...
    launch(nFeature, nFileDim, nTrees, numMapTasks, numThreads, mem, trainDataDir, workDir);
    return 0;
  }
    ...
    ExecutionException, ClassNotFoundException 
  {
    ...
    runDFREGTAV(nFeature, nFileDim, nTrees,
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);

    long endTime = System.currentTimeMillis();
    System.out
      .println("Total DFREGTAV Multi Dense Execution Time: "
        + (endTime - startTime));
  }

  private void runDFREGTAV(
    ...
    Job DFREGTAVJob =
      configureDFREGTAVJob(nFeature, nFileDim, nTrees,
		    numMapTasks, numThreads, mem, trainDataDir, dataDir, outDir, configuration);
    ...
    // ----------------------------------------------------------
    boolean jobSuccess =
      DFREGTAVJob.waitForCompletion(true);
    System.out
      .println("end Jod "
        + new SimpleDateFormat("HH:mm:ss.SSS")
          .format(Calendar.getInstance()
    ...
    // ---------------------------------------------------------
    if (!jobSuccess) {
      System.out.println("DFREGTAV Job fails.");
    }
  }

  private Job configureDFREGTAVJob(
    int nFeature, int nFileDim, int nTrees,
    int numMapTasks, int numThreads, int mem, 
    String trainDataDir, 
    Path dataDir,
    ...
    Job job =
      Job
        .getInstance(configuration, "DFREGTAV_job");
    FileInputFormat.setInputPaths(job, dataDir);
    FileOutputFormat.setOutputPath(job, outDir);
    job
      .setInputFormatClass(MultiFileInputFormat.class);
    job.setJarByClass(DFREGTAVDaalLauncher.class);
    job
      .setMapperClass(DFREGTAVDaalCollectiveMapper.class);
