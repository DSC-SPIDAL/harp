...
DMLC_REGISTRY_FILE_TAG(updater_dmatdensevec);

template<typename TStats>
class HistMakerCompactFastHistDenseVec: public BaseMaker {
 public:

 HistMakerCompactFastHistDenseVec(){
 ...
 protected:
  ...
  struct HistEntry {
    typename HistMakerCompactFastHistDenseVec<TStats>::HistUnit hist;
    unsigned istart;
...
//      /* OptApprox:: init bindid in pmat */
//      if (!this->isInitializedHistIndex){
//        CQHistMakerCompactFastHistDenseVec<TStats>::InitHistIndex(p_fmat, fset, tree);
//        this->isInitializedHistIndex = true;
//
//#ifdef   USE_COMPACT
//        p_hmat->Init(*p_fmat->GetSortedColumnBatches().begin(), p_fmat->Info());
...
XGBOOST_REGISTER_TREE_UPDATER(HistMakerCompactFastHistDenseVec, "grow_dmatdensevec")
.describe("Tree constructor that uses approximate global of histogram construction.")
.set_body([]() {
    return new HistMakerCompactFastHistDenseVec<GradStats>();
  });
}  // namespace tree
}  // namespace xgboost
